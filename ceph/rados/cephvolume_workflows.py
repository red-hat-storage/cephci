...
class CephVolumeWorkflows:
    ...
    def filesystem_exists_in_logical_volume(
        ...
        Usage:
            volumeobject_obj.filesystem_exists_in_logical_volume(osd_host=osd_host, osd_id=osd_id)
        """
    ...
    def get_osd_lvm_resource_name(self, osd_host, osd_id: str, resource: str) -> str:
        ...
        Usage:
            Name of logical volume:
                volumeobject_obj.get_osd_lvm_resource_name(osd_host=osd_host, osd_id=osd_id, resource="lv")
            Name of the volume group:
                volumeobject_obj.get_osd_lvm_resource_name(osd_host=osd_host, osd_id=osd_id, resource="vg")
        """
        lvm_list = self.lvm_list(host=osd_host, osd_id=osd_id)
        try:
            lvm_list = json.loads(lvm_list)
