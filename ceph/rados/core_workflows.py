...
EOF"""
    ...
    def remove_log_file_content(self, nodes_list, daemon_type=None):
        """
        ...
        cluster_fsid = self.run_ceph_command(cmd="ceph fsid")["fsid"]
        if daemon_type is None:
            cmd_truncate = rf"cd /var/log/ceph/{cluster_fsid}/ && find . -type f \
                            ! -name '*.gz' -exec truncate -s 0 {{}} \;"
        else:
            cmd_truncate = rf"cd /var/log/ceph/{cluster_fsid}/ && find . -type f -name '*{daemon_type}*' \
                            ! -name '*.gz' -exec truncate -s 0 {{}} \;"
