...
EOF"""
    ...
    def set_service_managed_type(self, service_type, unmanaged) -> bool:
        ...
        time.sleep(10)
        out = self.list_orch_services(service_type=service_type, export=True)[0]
        status = out.get("unmanaged", False)
        if status == "false":
        ...
        if unmanaged_check != unmanaged:
            ...
            log.info(f"{service_type} Service  in unmamaned={unmanaged} state. Pass")
            return True
