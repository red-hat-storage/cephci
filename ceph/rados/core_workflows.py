"""
rados_utils module is a configuration module for Ceph cluster.
...
class RadosOrchestrator:
    ...
    def configure_pg_autoscaler(self, **kwargs) -> bool:
        ...
        Args:
            **kwargs: Any other param that needs to be set
                1. enable -> enables PG_Autoscaler manager module (bool)
                2. pool_name -> If pool name is provided, sets the given value on the pool. (str)
    ...
    def enable_configuration_checks(self, configs: list):
        """
    ...
    def disable_configuration_checks(self, configs: list):
        """
    ...
    def enable_email_alerts(self, **kwargs) -> bool:
        ...
        try:
            ...
            log.error(
                "Error while enabling/disabling alerts module after configuration"
            )
            log.error(err)
            return False
    ...
    def create_erasure_pool(self, name: str, **kwargs) -> bool:
        ...
        Args:
            ...
            **kwargs: Any other param that needs to be set in the EC profile
                ...
                3. crush-failure-domain -> crush object to be us to store replica sets (str)
                4. plugin -> plugin to be set (str)
                    supported plugins:
                    1. jerasure (default)
                    2. isa
                    3. lrc
                    4. shec
                    5. clay
                5. pool_name -> pool name to create and associate with the EC profile being created
        Returns: True -> pass, False -> fail
    ...
    def pool_inline_compression(self, pool_name: str, **kwargs) -> bool:
        ...
        for val in value_map.keys():
            if kwargs.get(val, False):
                values = self.get_pool_property(pool=pool_name, props=val)
                if not re.search(value_map[val], values[val]):
                    log.error(f"Values do not match for config {val}")
                    return False
