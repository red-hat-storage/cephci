        ...
        if grouptype in {"single_pool_with_namespace", "multi_pool_with_namespace"}:
            if not kw.get("config", {}).get(pool_type, {}).get("group-namespace"):    
                if kw.get("is_secondary", False) is False:
                    ...
                    namespace = kw["config"][pool_type].get(pool,{}).get("namespace",None)
            else:
                ...
                namespace = kw.get("config", {}).get(pool_type, {}).get("group-namespace")
            kw["config"][pool_type].get(pool,{}).update({"remote_namespace":namespace})
            rc = create_namespace_and_verify(**{"pool-name":pool,"namespace":namespace,"client":client})
            if rc != 0:
                return rc 
        else:
        ...
        if kw.get("is_secondary", False) is False:
            if group_created != True:
                ...
                if rc != 0:
                    return rc
                group_created = True
                kw.get("config", {}).get(pool_type, {}).get(pool,{}).update({"group-spec": group_spec})
                kw.get("config", {}).get(pool_type, {}).update({"mirror-group" : group_spec})
                if namespace:
                    kw.get("config", {}).get(pool_type, {}).update({"group-namespace": namespace})
                imagelevel = kw.get("config", {}).get(pool_type, {}).get("mirror_level",{})
                kw.get("config", {}).get(pool_type, {}).get(pool,{}).update({"mirror_level": imagelevel})
            ...
            for imagename in multi_image_config.keys():
                ...
                mirror_group_spec = kw.get('config', {}).get(pool_type, {}).get('mirror-group','')
                # In case of multiple pools, images from all pools are added to the single group that was created on the first pool
                rc = add_image_to_group_and_verify(**{"group-spec":mirror_group_spec ,"image-spec": imagespec,"client" :client})
                if rc != 0:
                    return rc
