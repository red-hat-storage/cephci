...
from ceph.rbd.workflows.snap_clone_operations import (
    get_images_without_snap_and_or_clone,
)
...
def move_image_to_trash_and_verify(**kw):
    ...
    if expires_at:
        trash_conf.update({"expires-at": expires_at})
...
def purge_images_and_verify(**kw):
    ...
    if expired_before:
        purge_conf.update({"expired-before": expired_before})
    out, err = rbd.trash.purge(**purge_conf)
    if out or err and purge_err_msg not in out + err:
