...
class GenerateServiceSpec:
    ...
    def generate_nvmeof_spec(self, spec):
        ...
        enable_encryption = spec.pop("enable_encryption", None)
        encryption_key_path = spec.pop("encryption_key_path", None)
        nodes = get_nodes_by_ids(self.cluster, node_names)

        # encryption key
        if encryption and enable_encryption and encryption_key_path:
            ...
            dest_path = "/root/encryption.key"
            for node in nodes:
                node.exec_command(
                    cmd=f"printf '%s' '{key}' > {dest_path}",
                    sudo=True,
                )
            spec["spec"]["enable_encryption"] = True
            spec["spec"]["encryption_key_path"] = dest_path
        elif encryption:
            key_file = "encryption.key"
            installer_node = self.cluster.get_ceph_object("installer")
            installer_node.exec_command(
        ...
        if mtls:
            ...
            # From Ceph 9.1 onwards root_ca_cert, server_cert, server_key,
            # client_cert and client_key are not required. mtls key
            # Certificates are automatically created and managed by cephadm. Manually adding
            # certificate content to the specification file is not required. The certificates
            # generated by cephadm are located under /var/lib/ceph/FSID/nvmeof.CERTIFICATE
            if self.cluster.rhcs_version >= LooseVersion("9.1"):
                return template.render(spec=spec)
