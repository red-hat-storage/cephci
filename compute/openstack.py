...
class CephVMNodeV2:
    ...
    def shutdown(self, wait: bool = False) -> None:
        ...
        """
        try:
            LOG.info(f"Initiating shutdown of node: {self.node.name}")
            self.driver.ex_stop_node(self.node)
            if wait:
                try:
                    self._wait_until_vm_state(target_state="stopped")
                except NodeError as e:
                    LOG.error(f"Error while waiting for VM to stop with error: {e}")
                    raise
        except (ResourceNotFound, NetworkOpFailure, NodeError, VolumeOpFailure):
            raise
    ...
    def __setstate__(self, state) -> None:
        ...
        try:
            ...
            LOG.warning(f"Failed to restore OpenStack driver: {e}")
            self.driver = None
    ...
    def _wait_until_vm_state(self, target_state: str, timeout: int = 1200):
        ...
        raise NodeError(
            f"{node.name} did not reach {target_state}. Final state: {node.state}"
        )
