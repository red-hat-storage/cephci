...
def run_type_meta = [
    ...
    "rc": "RC build Sanity Run",
    "sanity_openstack_only": "PSI-Only Sanity Run"
    "schedule_openstack_only": "PSI-Only Schedule Run"
]
...
node(nodeName) {
    ...
    stage("PrepareTestStages") {
        ...
        run_type = "Manual Run"
        pipeline = tags_list.intersect(run_type_meta.keySet())
        if (pipeline){
            run_type = run_type_meta[pipeline[0]]
        }
        ...
        if (tags_list.any{element -> element.contains('openstack')}){
            is_openstack_run = true
            dirName = "psi_${currentBuild.projectName}_${currentBuild.number}"
            ...
            */
            recipeMap = sharedLib.readFromReleaseFile(
                majorVersion, minorVersion, lockFlag=false
            )
        }
        else{
            dirName = "ibm_${currentBuild.projectName}_${currentBuild.number}"
            recipeMap = sharedLib.readFromRecipeFile(rhcephVersion)
        }
    ...
    stage("AbortIfNewBuildAvailable"){
        if (tags_list.containsAll(["ibmc","sanity"]) && (tierLevel != "tier-0")){
            ...
            if ( buildArtifacts['ceph-version'] != content['ceph-version']) {
                umbLib.postUMBForPipeline(
                    rhcephVersion,
                    buildArtifacts["ceph-version"],
                    buildArtifacts,
                    tierLevel,
                    tags,
                    overrides,
    ...
    stage("PublishResults"){
        ...
        if(is_openstack_run){
            ...
            artifacts = buildArtifacts['recipe']
        }
        else{
