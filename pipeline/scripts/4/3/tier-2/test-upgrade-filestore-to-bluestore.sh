#! /bin/sh
echo "Beginning Red Hat Ceph RPM based cluster upgrade with ceph disk osd scenario and object store as filestore."

random_string=$(cat /dev/urandom | tr -cd 'a-z0-9' | head -c 5)
instance_name="ci-${random_string}"
platform="rhel-7"
rhbuild="4.3"
test_suite="suites/nautilus/upgrades/tier-2_upgrade_test-filestore-to-bluestore.yaml"
