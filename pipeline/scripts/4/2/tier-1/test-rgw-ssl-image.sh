#! /bin/sh
echo "Adding IO operations with SSL frontend on Container"

random_string=$(cat /dev/urandom | tr -cd 'a-z0-9' | head -c 5)
instance_name="ci-${random_string}"
platform="rhel-8"
rhbuild="4.2"
test_suite="suites/nautilus/rgw/tier-1_rgw_ssl_containerized-regression.yaml"
test_conf="conf/nautilus/rgw/tier-1_rgw.yaml"
