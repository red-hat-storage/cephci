#! /bin/sh
echo "Testing scale up and scale down functionality after upgrade from 4x cdn to 5x latest"

random_string=$(cat /dev/urandom | tr -cd 'a-z0-9' | head -c 5)
instance_name="ci-${random_string}"
platform="rhel-8"
rhbuild="5.1"
test_suite="suites/pacific/upgrades/tier-2_dmfg_test-elasticity-after-upgrade-from-4-cdn-to-5-latest.yaml"
