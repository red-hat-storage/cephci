...
class Mount(Cli):
    ...
    def nfs(self, mount, version, port, server, export, **kwargs):
        ...
        if is_krb_nfs4:
            ...
            if sec:
                opts.append(f"sec={sec}")
            # Non-Kerberos mounts require port=; callers may omit port (None) for 2049.
            mount_port = port if port is not None and str(port).strip() else "2049"
            opts.append(f"port={mount_port}")
