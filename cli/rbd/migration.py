...
class Migration(Cli):
    ...
    def prepare_import(self, **kw):
        ...
        cmd = (
            ...
            f"{dest_spec} --{cluster_name}"
        )
        return self.execute_as_sudo(cmd=cmd, long_running=True)

    def action_external(self, **kw):
        """
        Perform live migration as per the provided action for external ceph cluster,
        Args:
        kw(dict): Key/value pairs that needs to be provided to the client node.
            Example::
            Supported keys:
        ...
        dest_spec = kw.get("dest_spec", None)
        cluster_name = kw.get("cluster_name", None)
        log.info(f"Starting the {action} migration process to external ceph cluster")
