...
class Openstack:
    ...
    def get_nodes_by_pattern(self, pattern):
        """Get list of nodes by pattern

        Args:
            pattern (str): Pattern to be filtered for node name
        """
        nodes = [(n.name, n.id) for n in self._driver.list_nodes() if pattern in n.name]
        if nodes:
            return nodes
    ...
    def get_volumes_by_pattern(self, pattern):
        """Get list of volumes by pattern

        Args:
            pattern (str): Pattern to be filtered for volume name
        """
        volumes = [
            (v.name, v.id) for v in self._driver.list_volumes() if pattern in v.name
        ]
        if volumes:
            return volumes
    ...
    def create_ceph_nodes(self, cluster_conf, inventory, osp_cred, run_id, instances_name=None, enable_eus=False
    ):
