...
def run(ceph_cluster, **kw):
    ...
    * Export a file with specific permissions and confirm that the correct permissions are applied to the
    ...
    try:
        ...
        log.info("File Operations : move data b/w nfs mounts and local filesystem, \
        ...
        output, err = client1.exec_command(sudo=True, cmd=f"diff -qr {nfs_file_path} {nfs_file_copied_path}")
        output, err = client1.exec_command(sudo=True, cmd=f"diff -qr {local_file_path} {local_file_copied_path}")

        # Verify Data Integrity using MD5  of both the NFS file and the local file
        def calculate_md5(client, file_path):
            return client.exec_command(sudo=True, cmd=f"md5sum {file_path} | awk '{{print $1}}'")
        nfs_hash = calculate_md5(client1, nfs_file_path)
        local_hash = calculate_md5(client1, local_file_path)
        if nfs_hash == local_hash:
            print("File export and data integrity verification successful.")
        else:
            print("File export successful, but data integrity verification failed.")

        log.info("File Operations : Verify the file attributes are retained when a file is moved out of NFS Share, \
                        and also validate the same on a local file copied to NFS mount")
        def compare_file_attributes(attr1, attr2):
            return all(attr1[key] == attr2[key] for key in ['Permission', 'Uid', 'Gid', 'Size', 'Links'])
        src_nfs_file_attributes = fs_util.get_stats(clients[0], file_path=nfs_file_path, validate=True)
        des_nfs_file_attributes = fs_util.get_stats(clients[0], file_path=nfs_file_copied_path, validate=True)
        if compare_file_attributes(src_nfs_file_attributes, des_nfs_file_attributes):
            log.info('File Attributes on a NFS File are Retained after copying to local filesystem')
        else:
            log.info("File Attributes are not retained on a NFS File after copying to local filesystem")

        src_local_file_attributes = fs_util.get_stats(clients[0], file_path=local_file_path, validate=True)
        des_local_file_attributes = fs_util.get_stats(clients[0], file_path=local_file_copied_path, validate=True)
        if compare_file_attributes(src_local_file_attributes, des_local_file_attributes):
            log.info('File Attributes on a Local File are Retained after copying to NFS mount')
        else:
            log.info("File Attributes are not retained on a Local File after copying to NFS mount")

        log.info("Directory Operations : move directory b/w nfs mounts and local filesystem, \
        ...
        def clean_up_directory(client, directory_path):
            client.exec_command(sudo=True, cmd=f"rm -rf {directory_path}")
        clean_up_directory(client1, f"{nfs_mounting_dir}{nfs_dir}")
        clean_up_directory(client1, f"{local_fs_path}{nfs_dir}")

        def unmount_directory(client, directory_path):
            client.exec_command(sudo=True, cmd=f"umount -l {directory_path}", check_ec=False)
        unmount_directory(client1, nfs_mounting_dir)
        unmount_directory(client1, local_fs_path)

        def remove_export(client, nfs_name, nfs_export_name):
            client.exec_command(sudo=True, cmd=f"ceph nfs export delete {nfs_name} {nfs_export_name}", check_ec=False)
        remove_export(client1, nfs_name, nfs_export_name)

        def remove_nfs_cluster(client, nfs_name):
            client.exec_command(sudo=True, cmd=f"ceph nfs cluster rm {nfs_name}", check_ec=False)
        remove_nfs_cluster(client1, nfs_name)
