...
def run(ceph_cluster, **kw):
    ...
    nfs_to_restart = Ceph(clients[0]).nfs.cluster.ls() if instances_to_restart is None else instances_to_restart
    if not nfs_to_restart:
        raise ConfigError("No NFS instances found to restart. Please check the configuration.")

    time.sleep(20)  # Allow some time for the NFS cluster to stabilize before restarts
    ...
    if longevity and longevity_duration > 0:
        ...
        while (time.time() - start) < (longevity_duration * 3600):
            ...
            if restart_interval > 0:
                ...
                log.info(f"Waiting for {restart_interval} minutes before the next restart...")
