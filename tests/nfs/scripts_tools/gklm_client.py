...
class GklmClient:
    ...
    def __init__(self, host: str, port: int = 9443, user: str = None, password: str = None, verify: bool = False):
    ...
    def login(self) -> str:
        resp = requests.post(f"{self.base_url}/ckms/login",
    ...
    def logout(self) -> None:
        if self.authnId:
            requests.post(f"{self.base_url}/ckms/logout",
                          headers=self._headers(), verify=self.verify).raise_for_status()
        self.authnId = None
    ...
    def get_client_info(self, client_name) -> Dict[str, Any]:
        resp = requests.get(f"{self.base_url}/objects?clientName={client_name}",
                            headers=self._headers(), verify=self.verify)
        resp.raise_for_status()
        return resp.json()

    def create_client(self, client_name: str, application_usage: str = "Generic") -> Dict[str, Any]:
        """
        ...
        VALID_APP = {"Oracle", "MongoDB", "VMware", "FileNet", "NetApp", "Db2", "Generic"}
        ...
        resp = requests.post(f"{self.base_url}/clients", json=payload,
                             headers=self._headers(), verify=self.verify)
        try:
            resp.raise_for_status()
        except requests.HTTPError as e:
            msg = resp.json().get("message", resp.text) if resp.text else resp.text
            raise RuntimeError(f"Create client failed ({resp.status_code}): {msg}") from e
    ...
    def list_clients(self) -> List[Dict[str, Any]]:
        ...
        """
        resp = requests.get(f"{self.base_url}/clients", headers=self._headers(), verify=self.verify)
        ...
        try:
            ...
            raise RuntimeError(f"Failed to list clients ({resp.status_code}): {err_detail}") from e
    ...
    def delete_client(self, client_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                err_msg = resp.text
            raise RuntimeError(f"Delete client failed ({resp.status_code}): {err_msg}") from e
    ...
    def create_symmetric_key_object(
        ...
        try:
            ...
            raise RuntimeError(f"Create symmetric key failed ({resp.status_code}): {err}") from e
    ...
    def list_client_objects(
        ...
        resp = requests.get(f"{self.base_url}/objects",
                            headers=headers,
                            params=params,
                            verify=self.verify)
        try:
            ...
            try:
                ...
                err = resp.text
            raise RuntimeError(f"List objects failed ({resp.status_code}): {err}") from e
    ...
    def delete_object(self, object_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                err = resp.text or f"HTTP {resp.status_code}"
            raise RuntimeError(f"Delete object failed ({resp.status_code}): {err}") from e
    ...
    def assign_users_to_generic_kmip_client(
        ...
        try:
            ...
            try:
                ...
                err = resp.text or f"HTTP {resp.status_code}"
            raise RuntimeError(f"Assign users failed ({resp.status_code}): {err}") from e
        return resp.json()
    ...
    def list_certificates(
        ...
        try:
            ...
            try:
                ...
                err = resp.text
            raise RuntimeError(f"List certificates failed ({resp.status_code}): {err}") from e
    ...
    def delete_certificate(self, alias: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                err = resp.text or f"HTTP {resp.status_code}"
            raise RuntimeError(f"Delete certificate failed ({resp.status_code}): {err}") from e
