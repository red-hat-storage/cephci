...
def setup_nfs_cluster(
    ...
    for client in loop_clients:
        ...
        if run_user:
            ...
            out, _ = run_as_user(
                client, "ceph fs subvolumegroup ls cephfs", run_user=run_user
            )
...
def cleanup_cluster(
    clients, nfs_mount, nfs_name, nfs_export, nfs_nodes=None, run_user=None
):
    """
    ...
    for client in clients:
        ...
        if run_user:
            out, err = run_as_user(
                client, f"sudo umount {nfs_mount}", run_user=run_user, check_ec=False
            )
            if err:
                raise OperationFailedError(
    ...
    for i in range(len(clients)):
        if run_user:
            run_as_user(
                clients[0],
                f"ceph nfs export delete {nfs_name} {nfs_export}_{i}",
                run_user=run_user,
            )
        else:
    ...
    if run_user:
        run_as_user(
            clients[0], f"ceph nfs cluster delete {nfs_name}", run_user=run_user
        )
    else:
...
@retry(OperationFailedError, tries=4, delay=5, backoff=2)
def mount_retry(
    client, mount_name, version, port, nfs_server, export_name, run_user=None
):
    if run_user:
