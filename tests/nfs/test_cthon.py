...
def run_single_export(client, export, mount, nfs_node_ip, cthon_obj):
    """
    Run Cthon on a single export/mount.
    """
    try:
...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        def execute_all_clients():
            """
            Execute Cthon tests on all clients.
            This function randomly restarts the NFS cluster during execution
            to simulate real-world scenarios and ensure robustness.
            """
            if longevity:
                if random.random() < 0.7:  # 70% chance to restart
                    log.info(
                        "Randomly restarting NFS cluster during Cthon test execution..."
                    )
                    Ceph(clients[0]).orch.restart(f"nfs.{nfs_name}")
                    log.info(
                        f"Restarted NFS service on {clients[0].hostname} service nfs.{nfs_name}"
                    )
                else:
                    Ceph(clients[0]).orch.restart(f"nfs.{nfs_name}")
                    log.info(
