...
def run(ceph_cluster, **kw):
    ...
    7. repeat 1,2,3,4,5, and 6th steps in a loop - 10 or 20 times or for a given time in hours
    8. do not delete the cluster
    """
    ...
    longevity_duration = float(config.get("longevity_duration", 0))
    longevity_loop = config.get("longevity_loop", None)
    longevity = config.get("longevity", False)  # in hours
    ...
    nfs_cluster_name = config.get("cluster_name", None)
    ha = config.get("ha", False)
    vip = config.get("vip", None)
    loop_count = longevity_loop if longevity else loop_count
    ...
    try:
        if longevity and longevity_duration > 0:
            ...
            while (time.time() - start_time) < duration_seconds:
                ...
                log.info(
                    "NFS IO operations loop completed successfully. -- "
                    "remaining time: {:.2f} minutes".format(
                        (duration_seconds - (time.time() - start_time)) / 60
                    )
                )
            log.info("NFS IO operations completed successfully.")
            return 0
        else:
