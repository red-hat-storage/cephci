...
def _kdc_node_from_cluster(ceph_cluster, clients, config=None):
    config = config or {}
    idx = int(config.get("kdc_client_index", 1))
    if len(clients) > idx:
        return clients[idx]
...
def _build_kdc_setup(config, kdc_node):
    return MITKDCSetup(
        ...
        master_password=krb_config_get(
            config, "kdc_master_password", default=get_default_master_password()
        ),
...
def op_krb_deploy_mount_verify(
    ...
    if not check_mount_fails_without_ticket(client_node, mount_cmd):
...
def run(ceph_cluster, **kw):
    ...
    kdc_node = _kdc_node_from_cluster(ceph_cluster, all_clients, config)
    mount_path = krb_config_get(
        config, "krb_client_mount", "nfs_mount", default="/mnt/nfs_krb"
    )
    nfs_name = config.get("nfs_cluster_name", DEFAULT_NFS_KRB_CLUSTER)
