...
USER_KEYTAB_BASENAME = "cephci-krb-user.keytab"


def get_default_master_password():
    """Master KDC password: env override or ephemeral per-run secret."""
    return os.environ.get("CEPHCI_KDC_MASTER_PASSWORD") or secrets.token_urlsafe(32)


def _kadmin_safe_token(value):
    if not re.match(r"^[A-Za-z0-9._/@:-]+$", value or ""):
        raise ValueError("Unsafe Kerberos token: {!r}".format(value))
    return value
...
class MITKDCSetup:
    ...
    def __init__(
        ...
        self.node = node
        self.realm = _kadmin_safe_token(realm.upper())
        self.domain = domain.lower()
        self.kdc_hostname = kdc_hostname
        self.master_password = master_password or get_default_master_password()
        self.test_user = _kadmin_safe_token(test_user)
    ...
    def setup_kdc(self):
        ...
        try:
            ...
            try:
                ...
                log.warning("KDC cleanup after setup failure: %s", cleanup_ex)
            raise

    def add_user_principal(self, username=None):
        user = _kadmin_safe_token(username or self.test_user)
        principal = "{}@{}".format(user, self.realm)
        self._ensure_randkey_principal(principal)
        return principal

    def export_user_keytab(self, username=None, remote_path=None):
        user = _kadmin_safe_token(username or self.test_user)
        principal = "{}@{}".format(user, self.realm)
        if remote_path is None:
            safe_user = re.sub(r"[^a-zA-Z0-9._-]", "_", user)
            remote_path = "/tmp/cephci-user-{}.keytab".format(safe_user)
        self.export_keytab_for_principal(principal, remote_path=remote_path)
        return remote_path

    def add_nfs_service_principal(self, nfs_fqdn):
        spn = "nfs/{}@{}".format(_kadmin_safe_token(nfs_fqdn), self.realm)
        self._ensure_randkey_principal(spn)
        return spn

    def add_host_service_principal(self, host_fqdn):
        """Host principal so RHEL starts ``rpc-gssd`` (requires ``/etc/krb5.keytab``)."""
        spn = "host/{}@{}".format(_kadmin_safe_token(host_fqdn), self.realm)
        self._ensure_randkey_principal(spn)
        return spn
    ...
    def export_keytab_for_principal(
        ...
        self.node.exec_command(
            sudo=True,
            cmd="chmod 600 {}".format(shlex.quote(remote_path)),
            check_ec=False,
        )
    ...
    def _setup_kdc_host(self):
        ...
        if not _remote_test(self.node, marker):
            ...
            pw_handle = self.node.remote_file(
                sudo=True, file_name=pw_file, file_mode="w"
            )
            pw_handle.write(self.master_password)
            pw_handle.flush()
            pw_handle.close()
            self.node.exec_command(
                sudo=True,
                cmd="chmod 600 {}".format(shlex.quote(pw_file)),
                check_ec=False,
            )
            self.node.exec_command(
                sudo=True,
                cmd=(
                    "bash -c 'pw=$(cat {pw_file}); "
                    'printf "%s\\n%s\\n" "$pw" "$pw" | kdb5_util create -s\''
                ).format(pw_file=shlex.quote(pw_file)),
            )
            self.node.exec_command(
                sudo=True,
    ...
    def _ensure_randkey_principal(self, principal):
        """Create principal with -randkey; tolerate 'already exists', fail on real errors."""
        out, err = self._kadmin_local(
            "addprinc -randkey {}".format(principal), check_ec=False
        )
        text = "{}\n{}".format(out or "", err or "")
        lowered = text.lower()
        if "principal already exists" in lowered or "added principal" in lowered:
            log.info("Ensured principal %s (randkey)", principal)
            return
        if "cannot find/read master key" in lowered or "kadm5" in lowered:
            raise RuntimeError(
                "kadmin addprinc failed for {}: {}".format(principal, text.strip())
            )
        if text.strip():
            log.warning(
                "Unexpected kadmin output for %s (continuing): %s",
                principal,
                text.strip()[:200],
            )
        log.info("Ensured principal %s (randkey)", principal)
...
def krb5_conf_content(realm, domain, kdc_hostname, kdc_ip=None):
