...
class MITKDCSetup:
    ...
    def add_user_principal(self, username=None):
        ...
        self._kadmin_local("addprinc -randkey {}".format(principal), check_ec=False)
        log.info("Ensured user principal %s (randkey)", principal)
        return principal
    ...
    def _setup_kdc_host(self):
        ...
        if not _remote_test(self.node, marker):
            ...
            self.node.exec_command(
                sudo=True,
                cmd=("bash -c 'kdb5_util create -s < {pw_file}'").format(
                    pw_file=shlex.quote(pw_file)
                ),
            )
            self.node.exec_command(
                sudo=True,
