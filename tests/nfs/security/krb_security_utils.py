...
from tests.nfs.security.kerberos_helper import krb5_conf_content
...
def install_krb_client_packages(node, nfs_server=False):
    ...
    _configure_nfs_conf_gssd_use_proxy(node, use_proxy=bool(nfs_server))
...
def install_user_keytab(kdc_setup, client_node, keytab_path=USER_KEYTAB_PATH):
    """Install the test-user keytab for ``kinit -kt`` on NFS clients."""
    remote_kt = kdc_setup.export_user_keytab()
    keytab_bytes = kdc_setup.fetch_keytab_bytes(remote_path=remote_kt)
    install_user_keytab_bytes(client_node, keytab_bytes, keytab_path=keytab_path)


def install_user_keytab_bytes(client_node, keytab_bytes, keytab_path=USER_KEYTAB_PATH):
    handle = client_node.remote_file(sudo=True, file_name=keytab_path, file_mode="wb")
    handle.write(keytab_bytes)
    handle.flush()
    handle.close()
    client_node.exec_command(
        sudo=True,
        cmd="chmod 600 {} && chown root:root {}".format(
            shlex.quote(keytab_path), shlex.quote(keytab_path)
        ),
        check_ec=False,
    )
    log.info("Installed user keytab on %s at %s", client_node.hostname, keytab_path)
...
def verify_kdc_kinit_from_node(
    node, kdc_setup, keytab_path=USER_KEYTAB_PATH, timeout=60, interval=5
):
    """
    Confirm the KDC answers krb5 AS-REQ from ``node`` (UDP/TCP 88).

    Uses ``kinit -kt`` with the distributed user keytab (no password in shell).
    """
    ...
    for w in WaitUntil(timeout=timeout, interval=interval):
        ...
        _, err = node.exec_command(
            sudo=True,
            cmd="kinit -kt {} {}".format(
                shlex.quote(keytab_path), shlex.quote(principal)
            ),
            check_ec=False,
        )
        ...
        log.warning(
            "KDC kinit from %s attempt failed: %s",
            node.hostname,
            last_err,
        )
    raise OperationFailedError(
...
def check_mount_fails_without_ticket(client_node, mount_cmd):
    """Verify mount fails without a user ``kinit`` ticket.

    Returns:
        True: mount failed as expected (no NFS mount present).
        False: mount unexpectedly succeeded without a user ticket.

    Raises:
        OperationFailedError: mount failed for the wrong reason (e.g. invalid
            mount option before Kerberos client services are ready).
    """
    client_node.exec_command(sudo=True, cmd="kdestroy -A", check_ec=False)
    ...
    try:
        ...
        out = ""
        saw_invalid_option = False
        for idx, cmd in enumerate(attempts):
            out = _mount_stderr(client_node, cmd)
            if _is_invalid_mount_option_error(out):
                saw_invalid_option = True
                if idx < len(attempts) - 1:
                    log.warning(
                        "Kerberos mount rejected options (%s); retrying with "
                        "mount.nfs nfsvers fallback",
                        out.strip()[:160],
                    )
                continue
            break
        else:
            if saw_invalid_option:
                raise OperationFailedError(
                    "Kerberos mount failed with invalid mount option on all "
                    "attempts (nfs4 and nfsvers fallback). Last output: {}".format(
                        out.strip()
                    )
                )
...
def wait_for_no_keytab_errors_in_nfs_logs(
    ...
    for w in WaitUntil(timeout=timeout, interval=interval):
        ...
        if logs_ok:
            log.info(
                "No keytab CRIT patterns in NFS logs after ~%ss",
                int(time.time() - start),
            )
            return True
...
def provision_kerberos_environment(
    ...
    user_kt = kdc_setup.export_user_keytab()
    verify_kdc_kinit_from_node(kdc_node, kdc_setup, keytab_path=user_kt)
    user_keytab_bytes = kdc_setup.fetch_keytab_bytes(remote_path=user_kt)

    for client_node in client_nodes:
        install_user_keytab_bytes(client_node, user_keytab_bytes)
        client_fqdn = node_fqdn(client_node, domain)
