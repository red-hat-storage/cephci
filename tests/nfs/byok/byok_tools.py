...
def get_enctag(
    ...
    try:
        ...
        all_clients = gklm_client.clients.list_clients()
        [gkml_client_name_data] = [
            client
            for client in all_clients
            if client["clientName"] == gkml_client_name.upper()
        ]

        if [gklm_user] == gkml_client_name_data.get("users"):
            log.info(
                f"Assigning user '{gklm_user}' to GKLM client "
                f"'{gkml_client_name}' for KMIP access Since user is not was not assigned"
            )
            gklm_client.clients.assign_users_to_generic_kmip_client(
                client_name=gkml_client_name, users=[gklm_user]
            )
            gklm_client.clients.assign_client_certificate(
                client_name=gkml_client_name, cert_pem=cert, alias=gklm_cert_alias
            )
