...
def reboot_node_with_wait_and_loops(node, wait=10, loops=1):
    ...
    for i in range(loops):
        log.info(f"Starting reboot {i + 1} of {loops} for node {node.hostname}")
        reboot_node(node)
        log.info(f"Reboot {i + 1} completed for node {node.hostname}")
        if loops > 1 and i < loops - 1:  # Don't wait after the last reboot
            log.info(f"Waiting {wait} seconds before next reboot...")
            sleep(wait)
            log.info("Wait period completed, proceeding with next reboot")

    log.info(f"All {loops} reboots completed for node {node.hostname}")
    ...
    try:
        ...
        for client in clients_list:
            if client.get("name", "") == gkml_client_name:
                ...
                log.info(f"GKLM client {gkml_client_name} already exists")
                break

        if not is_gklm_client_present:
            log.info(f"Creating GKLM client {gkml_client_name}")
