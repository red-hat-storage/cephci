...
gRPC auth depends on Ceph version:
  - >= 20.2.2: mTLS (certs from admin node
    /var/lib/ceph/<fsid>/nfs_grpc-client-certs/nfs.<nfs-cluster-name>/)
  - < 20.2.2: plaintext grpcurl
...
# Bound grpcurl so a wedged/firewalled endpoint cannot hang the suite
GRPCURL_TIMEOUT = 120
# NFS gRPC became mTLS-secured starting with Ceph 20.2.2 builds
GRPC_MTLS_MIN_VERSION = "20.2.2"
...
def grpc_requires_mtls(ceph_version):
    """
    Return True when NFS gRPC requires mTLS for this Ceph build.

    Ceph >= 20.2.2 (e.g. 20.2.2-395) uses mTLS.
    Older builds (e.g. 20.2.1-400) use plaintext.
    """
    if not ceph_version:
        return False
    return LooseVersion(str(ceph_version)) >= LooseVersion(GRPC_MTLS_MIN_VERSION)


def resolve_ceph_version(client_node, admin_node=None):
    """Resolve installed Ceph version from client, falling back to admin/cephadm."""
    for node, prefix in (
        (client_node, False),
        (admin_node, True),
    ):
        if node is None:
            continue
        try:
            version = get_ceph_version(node, prefix_cephadm=prefix)
            if version:
                return version
        except Exception as exc:
            log.warning(
                "Failed to get Ceph version from %s (prefix_cephadm=%s): %s",
...
def grpcurl_conn_flags(cert_dir=None, use_mtls=False):
    """Return grpcurl connection flags: mTLS certs or -plaintext."""
    if use_mtls:
        if not cert_dir:
            raise OperationFailedError(
                "mTLS gRPC requested but cert_dir was not provided"
            )
        return grpc_auth_flags(cert_dir)
    return "-plaintext"
...
def probe_grpc_connectivity(client_node, nfs_ip, cert_dir=None, use_mtls=False):
    """
    Lightweight gRPC connectivity probe: `grpcurl list` must return >=1 service.

    Uses mTLS or plaintext based on use_mtls. Unlike discovery, this does not
    assert the full expected service set.
    """
    mode = "mTLS" if use_mtls else "plaintext"
    log.info(f"Probing {mode} gRPC connectivity on {nfs_ip}:{GRPC_PORT}")
    cmd = (
        f"grpcurl {grpcurl_conn_flags(cert_dir, use_mtls)} "
        f"{nfs_ip}:{GRPC_PORT} list"
    )
    out, err = client_node.exec_command(
        sudo=True, cmd=cmd, check_ec=False, timeout=GRPCURL_TIMEOUT
    )
...
probe_grpc_mtls = probe_grpc_connectivity


def verify_grpc_service_discovery(client_node, nfs_ip, cert_dir=None, use_mtls=False):
    """Confirm expected gRPC services via grpcurl list (mTLS or plaintext)."""
    mode = "mTLS" if use_mtls else "plaintext"
    log.info(f"Discovering gRPC services on {nfs_ip}:{GRPC_PORT} ({mode})")

    ok, result = probe_grpc_connectivity(
        client_node, nfs_ip, cert_dir=cert_dir, use_mtls=use_mtls
    )
    if not ok:
...
def start_grace_period(
    client_node, nfs_ip, event_id, cert_dir=None, use_mtls=False, node_id=1
):
    """
    ...
    cmd = (
        f"grpcurl {grpcurl_conn_flags(cert_dir, use_mtls)} -d '{request_data}' "
...
def verify_grace_logs(nfs_node, nfs_name, event_id, client, log_before=""):
    ...
    Requires a non-empty log_before snapshot and that pattern occurrence
    count increases after the event (avoids matching stale startup lines).
    """
    ...
    if not str(log_before or "").strip():
        log.error(
            "Empty/missing log_before snapshot; cannot verify new grace log lines"
        )
        return False
    ...
    for pattern in patterns:
        before_count = log_before.count(pattern)
...
def get_client_ids(client_node, nfs_ip, cert_dir=GRPC_CERT_DIR_CLIENT, use_mtls=False):
    """Get NFS client IDs via gRPC. Returns (ok, list)."""
...
def get_session_ids(client_node, nfs_ip, cert_dir=GRPC_CERT_DIR_CLIENT, use_mtls=False):
    """Get NFS session IDs via gRPC. Returns (ok, list)."""
...
def run(ceph_cluster, **kw):
    ...
    admin_node = installer_nodes[0] if installer_nodes else None
    ...
    try:
        ...
        ceph_version = resolve_ceph_version(client, admin_node)
        use_mtls = grpc_requires_mtls(ceph_version)
        log.info(
            "Ceph version %s -> NFS gRPC mode: %s (mTLS min %s)",
            ceph_version,
            "mTLS" if use_mtls else "plaintext",
            GRPC_MTLS_MIN_VERSION,
        )

        if use_mtls:
            if admin_node is None:
                raise OperationFailedError(
                    "No installer/admin node available for gRPC mTLS certs"
                )
        ...
        if operation == "verify_port":
            ...
            ok, result = probe_grpc_connectivity(
                client, nfs_ip, cert_dir=cert_dir, use_mtls=use_mtls
            )
            if not ok:
                mode = "mTLS" if use_mtls else "plaintext"
                raise OperationFailedError(
                    f"gRPC port listening but {mode} probe failed on "
                    f"{nfs_ip}:{GRPC_PORT}: {result}"
                )
            mode = "mTLS" if use_mtls else "plaintext"
            log.info(
                f"PASS: gRPC port {GRPC_PORT} listening and reachable over {mode} "
            ...
            success, services = verify_grpc_service_discovery(
                client, nfs_ip, cert_dir=cert_dir, use_mtls=use_mtls
            )
            if not success:
                raise OperationFailedError(
            ...
            if not str(log_before).strip():
                raise OperationFailedError(
                    "Could not snapshot NFS daemon logs before grace event"
                )
            ...
            success, client_ids = get_client_ids(
                client, nfs_ip, cert_dir=cert_dir, use_mtls=use_mtls
            )
            if not success:
                raise OperationFailedError("Could not get client IDs via gRPC")

            success, session_ids = get_session_ids(
                client, nfs_ip, cert_dir=cert_dir, use_mtls=use_mtls
            )
            if not success:
            ...
            success, initial_client_ids = get_client_ids(
                client, nfs_ip, cert_dir=cert_dir, use_mtls=use_mtls
            )
            if not success:
                raise OperationFailedError("Could not get initial client IDs via gRPC")
            success, initial_session_ids = get_session_ids(
                client, nfs_ip, cert_dir=cert_dir, use_mtls=use_mtls
            )
            if not success:
            ...
            success, updated_client_ids = get_client_ids(
                client, nfs_ip, cert_dir=cert_dir, use_mtls=use_mtls
            )
            if not success:
                raise OperationFailedError("Could not get updated client IDs via gRPC")
            success, updated_session_ids = get_session_ids(
                client, nfs_ip, cert_dir=cert_dir, use_mtls=use_mtls
            )
            if not success:
        ...
        log.info(
            "TEST PASSED - operation=%s nfs=%s grpc_mode=%s checks completed",
            operation,
            nfs_name,
            "mTLS" if use_mtls else "plaintext",
        )
