...
def run(ceph_cluster, **kw):
    ...
    if not nfs_clusters:
        if not auto_create_nfs_cluster:
            raise ConfigError(
                "No NFS cluster found. Set auto_create_nfs_cluster=true or pre-deploy nfs cluster."
            )
        nfs_servers = [node.hostname for node in nfs_nodes]
        log.info(
            "No NFS cluster found. Creating %s on nfs nodes: %s",
            nfs_name,
            ", ".join(nfs_servers),
        )
        setup_nfs_cluster(
    ...
    try:
        backup_exists = backup_current_template(nfs_cmd_host)
        if enable_debug_logging:
            log.info(
                "Enabling Ganesha FULL_DEBUG logging block before delegation validation"
            )
            enable_ganesha_debug_logging(nfs_cmd_host)
            redeploy_nfs_clusters(
        ...
        for delegation in delegation_sequence:
            ...
            for cluster_name in nfs_clusters:
                ...
                verify_container_delegation(
                    nfs_cmd_host, nfs_nodes, daemons, cluster_name, delegation
                )
        ...
        try:
            if reset_to_default_on_exit:
                ...
                restore_previous_template(nfs_cmd_host, backup_exists)
            redeploy_nfs_clusters(
                cephadm, nfs_clusters, installer, redeploy_wait, service_wait_timeout
            )
            if created_cluster:
                cleanup_created_nfs_cluster(
                    clients, nfs_cmd_host, nfs_mount, nfs_name, nfs_export
                )
