...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        num_of_mons = len(fs_util_ceph1.get_mon_node_ips())
        ...
        # nfs_cluster = Ceph(source_clients[0]).nfs.cluster
        # out = nfs_cluster.create(
        #     name=nfs_name,
            ...
            try:
                ...
                if test_type == "signal":
                    ...
                    for mds in active_mds_objects:
                        log.info(
                            "Sending %s to MDS on host: %s", test_name, mds.hostname
                        )
                        fs_util_v1_ceph1.pid_signal(
                        ...
                        if sig == signal.SIGTERM and expect_exit:
                            log.info(
                                "Restarting MDS service after SIGTERM on host: %s",
                                mds.hostname,
                            )
                            service_name = fs_util_v1_ceph1.deamon_name(
                                mds, rf"mds\.{source_fs}\."
                            )
                            ...
                            if "active" in out:
                                log.info(
                                    "MDS service %s started successfully", service_name
                                )
                            else:
                                log.warning(
                                    "MDS service %s might not be active: %s",
                                    service_name,
                                    out,
                                )
                    ...
                    for mds in active_mds_objects:
                        log.info(
                            "Restarting MDS using systemctl on host: %s", mds.hostname
                        )
                    ...
                    for mds in active_mds_objects:
                        ...
                        fs_util_v1_ceph1.reboot_node_v1(mds, timeout=300)
                    ...
                    try:
                        ...
                        log.info(
                            "Current MDS daemons running on hosts: %s",
                            list(current_mds_hostnames),
                        )
                    ...
                    if not available_for_deploy:
                        ...
                        log.info(
                            "Nodes available for MDS redeploy: %s",
                            list(available_for_deploy),
                        )

                        # Select nodes for redeploy (use first available node, or all if needed)
                        nodes_to_deploy = list(available_for_deploy)[
                            -len(current_mds_hostnames) :
                        ]
                        ...
                        deploy_cmd = (
                            f"ceph orch apply mds {source_fs} "
                            f"--placement='{len(current_mds_hostnames)} {hosts_str}'"
                        )
                        ...
                        mds_process_name = f"mds.{source_fs}"
                        if wait_for_process(
                            client=source_clients[0],
