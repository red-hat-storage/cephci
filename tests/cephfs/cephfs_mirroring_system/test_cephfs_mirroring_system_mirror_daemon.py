...
def run_cephfs_mirror_signal_tests(
    fs_util_v1,
    cephfs_mirror_nodes,
    test_case,
):
    """
    Execute SIGHUP, SIGTERM, and SIGKILL signal tests on cephfs-mirror daemons.

    This function sends signals to active cephfs-mirror daemons and handles service
    restart for SIGTERM.
    ...
    try:
        ...
        log.info("Sending %s to cephfs-mirror daemons", test_name)

        # Send signal to cephfs-mirror daemons
        for mirror_node in cephfs_mirror_nodes:
            node = mirror_node.node
            log.info(
                "Sending %s to cephfs-mirror on host: %s",
                test_name,
                node.hostname,
            )
            fs_util_v1.pid_signal(
                node, "cephfs-mirror", sig=sig, expect_exit=expect_exit, wait=10
            )
            # If SIGTERM was sent and expect_exit is True, restart the service
            if sig == signal.SIGTERM and expect_exit:
                log.info(
                    "Restarting cephfs-mirror service after SIGTERM on host: %s",
                    node.hostname,
                )
                # Get service name for cephfs-mirror
                service_name = fs_util_v1.deamon_name(node, r"cephfs-mirror\.")
                log.info("Starting cephfs-mirror service: %s", service_name)
                node.exec_command(
                    sudo=True,
                ...
                if "active" in out:
                    log.info(
                        "cephfs-mirror service %s started successfully", service_name
                    )
                else:
                    log.warning(
                        "cephfs-mirror service %s might not be active: %s",
                        service_name,
                        out,
                    )

        # Wait for cephfs-mirror to recover
        log.info("Waiting for cephfs-mirror to recover after %s", test_name)
        time.sleep(30)
...
def run_cephfs_mirror_systemctl_restart_test(
    fs_util_v1,
    cephfs_mirror_nodes,
):
    """
    Execute systemctl restart test on cephfs-mirror daemons.

    This function restarts cephfs-mirror daemons using systemctl restart.
    ...
    """
    try:
        log.info("Restarting cephfs-mirror daemons using systemctl restart")

        # Restart cephfs-mirror using systemctl restart
        for mirror_node in cephfs_mirror_nodes:
            node = mirror_node.node
            log.info(
                "Restarting cephfs-mirror using systemctl on host: %s",
                node.hostname,
            )
            # Get cephfs-mirror service name using deamon_name method
            service_name = fs_util_v1.deamon_name(node, r"cephfs-mirror\.")
            log.info("Restarting cephfs-mirror service: %s", service_name)
            node.exec_command(
                sudo=True,
        ...
        # Wait for cephfs-mirror to recover
        log.info("Waiting for cephfs-mirror to recover after systemctl restart")
        time.sleep(30)
...
def run_cephfs_mirror_node_reboot_test(
    fs_util_v1,
    cephfs_mirror_nodes,
):
    """
    Execute node reboot test on cephfs-mirror nodes.

    This function reboots cephfs-mirror nodes.
    ...
    """
    try:
        log.info("Rebooting cephfs-mirror nodes")
        for mirror_node in cephfs_mirror_nodes:
            node = mirror_node.node
            log.info("Rebooting cephfs-mirror node: %s", node.hostname)
            fs_util_v1.reboot_node_v1(node, timeout=300)
        # Wait for cephfs-mirror to recover after reboot
        log.info("Waiting for cephfs-mirror to recover after node reboot")
        time.sleep(60)
...
def run_cephfs_mirror_container_restart_test(
    cephfs_mirror_nodes,
):
    """
    Execute container restart test on cephfs-mirror daemons using podman restart.

    This function restarts cephfs-mirror containers using podman restart.
    ...
    """
    try:
        log.info("Restarting cephfs-mirror containers using podman restart")
        for mirror_node in cephfs_mirror_nodes:
            node = mirror_node.node
            log.info("Restarting cephfs-mirror container on host: %s", node.hostname)
            try:
                # Get cephfs-mirror container ID using podman ps | grep cephfs-mirror
                out, _ = node.exec_command(
                    sudo=True,
                    cmd="podman ps | grep cephfs-mirror",
                )
                # Extract container ID (first column)
                lines = out.strip().split("\n")
                for line in lines:
                    if "cephfs-mirror" in line:
                        container_id = line.split()[0]
                        log.info(
                            "Restarting cephfs-mirror container: %s on host: %s",
                            container_id,
                            node.hostname,
                        )
                        node.exec_command(
                ...
                log.warning(
                    "Failed to restart cephfs-mirror container on %s: %s",
                    node.hostname,
                    e,
                )
        # Wait for cephfs-mirror to recover
        log.info("Waiting for cephfs-mirror to recover after container restart")
        time.sleep(30)
...
def run_cephfs_mirror_redeploy_test(
    client,
    ceph_cluster_dict,
    cluster_key,
):
    """
    Execute cephfs-mirror redeploy test by deploying cephfs-mirror on different nodes.

    Args:
        client: Client node to execute commands on
        ceph_cluster_dict: Dictionary containing cluster objects
        cluster_key: Key for the cluster in ceph_cluster_dict (e.g., "ceph1")

    Returns:
        int: 0 if successful, 1 if there's an error
    """
    # Get current cephfs-mirror daemon hostnames
    log.info("Getting current cephfs-mirror daemon hostnames")
    ...
    current_mirror_hostnames = set()
    try:
        mirror_data = json.loads(out)
        for daemon in mirror_data:
            hostname = daemon.get("hostname")
            if hostname:
                current_mirror_hostnames.add(hostname)
        log.info(
            "Current cephfs-mirror daemons running on hosts: %s",
            list(current_mirror_hostnames),
        )
    except json.JSONDecodeError as e:
        log.error("Failed to parse cephfs-mirror daemon information: %s", e)
        return 1

    # Get all available cephfs-mirror nodes Get mds role as only one host added with cephfs-mirror role.
    mirror_hosts = ceph_cluster_dict.get(cluster_key).get_nodes(role="mds")
    if not mirror_hosts:
        log.warning("No cephfs-mirror nodes found in cluster")
        return 0

    all_mirror_hostnames = {node.hostname for node in mirror_hosts}
    log.info("All available cephfs-mirror nodes: %s", list(all_mirror_hostnames))

    # Find nodes that are NOT currently running cephfs-mirror
    available_for_deploy = all_mirror_hostnames - current_mirror_hostnames

    if not available_for_deploy:
        log.warning(
            "No available nodes for cephfs-mirror redeploy. "
            "All cephfs-mirror nodes are currently running cephfs-mirror."
        )
        log.info("Skipping cephfs-mirror redeploy test")
    else:
        log.info(
            "Nodes available for cephfs-mirror redeploy: %s", list(available_for_deploy)
        )

        # Select nodes for redeploy
        nodes_to_deploy = list(available_for_deploy)[: len(current_mirror_hostnames)]
        log.info("Deploying cephfs-mirror on nodes: %s", nodes_to_deploy)

        # Deploy cephfs-mirror on selected nodes
        hosts_str = " ".join(nodes_to_deploy)
        deploy_cmd = f"ceph orch apply cephfs-mirror --placement='{len(current_mirror_hostnames)} {hosts_str}'"
        ...
        log.info("cephfs-mirror deployment command executed successfully")

        # Wait for cephfs-mirror to be deployed and become active
        log.info("Waiting for cephfs-mirror to be deployed on new nodes")
        mirror_process_name = "cephfs-mirror"
        if wait_for_process(
            client=client,
            process_name=mirror_process_name,
            timeout=120,
            interval=10,
            ispresent=True,
        ):
            log.info("cephfs-mirror redeploy completed successfully")
        else:
            log.warning("cephfs-mirror deployment verification timeout")

    # Wait for cephfs-mirror to stabilize
    log.info("Waiting for cephfs-mirror to stabilize after redeploy")
    time.sleep(30)
...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        if not cephfs_mirror_nodes:
            log.error("No cephfs-mirror nodes found in ceph1 cluster")
            return 1
        log.info(
            "Found %s cephfs-mirror node(s): %s",
            len(cephfs_mirror_nodes),
            cephfs_mirror_nodes,
        )
        ...
        for test_case in signal_tests:
            ...
            try:
                ...
                if test_type == "signal":
                    # Run signal tests on cephfs-mirror daemons
                    log.info("Running signal test on cephfs-mirror daemons")
                    result = run_cephfs_mirror_signal_tests(
                        fs_util_v1_ceph1,
                        cephfs_mirror_nodes,
                        test_case,
                    )
                    ...
                    # Run systemctl restart test on cephfs-mirror daemons
                    log.info("Running systemctl restart test on cephfs-mirror daemons")
                    result = run_cephfs_mirror_systemctl_restart_test(
                        fs_util_v1_ceph1,
                        cephfs_mirror_nodes,
                    )
                    if result != 0:
                    ...
                    # Run container restart test on cephfs-mirror daemons
                    log.info("Running container restart test on cephfs-mirror daemons")
                    result = run_cephfs_mirror_container_restart_test(
                        cephfs_mirror_nodes,
                    )
                    if result != 0:
                        return 1
                    ...
                    # Run node reboot test on cephfs-mirror nodes
                    log.info("Running node reboot test on cephfs-mirror nodes")
                    result = run_cephfs_mirror_node_reboot_test(
                        fs_util_v1_ceph1,
                        cephfs_mirror_nodes,
                    )
                    if result != 0:
                    ...
                    # Run cephfs-mirror redeploy test
                    log.info("Running cephfs-mirror redeploy test")
                    result = run_cephfs_mirror_redeploy_test(
                        source_clients[0],
                        ceph_cluster_dict,
                        "ceph1",
                    )
            ...
            if needs_asok_refetch:
                ...
                # cephfs_mirror_nodes = ceph_cluster_dict.get("ceph1").get_ceph_object("mds",0)
                # cephfs_mirror_nodes = [daemon_name[0].split(".")[1]]
                cephfs_mirror_nodes = ceph_cluster_dict.get(
                    "ceph1"
                ).get_ceph_objects_by_hostname(daemon_name[0].split(".")[1])
                # Refetch asok_file
                asok_file = fs_mirroring_utils.get_asok_file_with_connectivity_check(
                    cephfs_mirror_nodes, fsid, daemon_name
                )
            ...
            # Fetch last_synced_snap.id before the test execution, so we can compare after the test
            snap_sync_id_before = []
            for path in subvolume_paths:
                sync_id = fs_mirroring_utils.get_last_synced_snap_id(
                    source_fs, fsid, asok_file, filesystem_id, peer_uuid, path
                ...
                if sync_id is not None:
                    ...
                    log.error(
                        "Could not retrieve last_synced_snap id (before) for %s during %s",
                        path,
                        test_name,
                    )
                    return 1

            # Wait for snapshot sync to progress after test (already waited above)

            # After test execution, fetch last_synced_snap.id and compare

            from utility.retry import retry

            @retry(Exception, tries=5, delay=15)
            def wait_for_sync_id_increase(idx, path, sync_id_before):
                """
                Retry fetching last_synced_snap.id and wait for it to increase.
                Returns the after sync id if successful, raises Exception if not.
                """
                sync_id_after = fs_mirroring_utils.get_last_synced_snap_id(
                    source_fs, fsid, asok_file, filesystem_id, peer_uuid, path
                )
                if sync_id_after is not None:
                    if sync_id_after > sync_id_before:
                        log.info(
                    ...
                    raise Exception(
                        f"Could not retrieve last_synced_snap id (after) for {path} during {test_name}"
                    )
