...
class FsUtils(object):
    ...
    def create_fs(self, client, vol_name, validate=True, **kwargs):
        ...
        log.info(f"Validating the creation of pools")
        outpools, rcpools = client.exec_command(sudo=True, cmd=f"ceph osd lspools | grep -E 'cephfs.{vol_name}.data|cephfs.{vol_name}.meta' | wc -l")
        if outpools.strip() != "2":
            raise CommandFailed(f"Creation of pools: {vol_name} failed. Actual Output: {outpools}")
    ...
    def remove_fs(self, client, vol_name, validate=True, **kwargs):
        ...
        if validate:
            ...
            log.info(f"Validating the deletion of pools")
            outpools, rcpools = client.exec_command(sudo=True, cmd=f"ceph osd lspools | grep -E 'cephfs.{vol_name}.data|cephfs.{vol_name}.meta' | wc -l")
            if outpools.strip() != "0":
                raise CommandFailed(f"Deletion of pools: {vol_name} failed. Actual Output: {outpools}")
