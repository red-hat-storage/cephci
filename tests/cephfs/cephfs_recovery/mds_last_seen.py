...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        client1.exec_command(
            sudo=True, cmd="ceph fs set %s allow_standby_replay true" % fs_name
        )
        client1.exec_command(
        ...
        time.sleep(30)
        log.info("==== COMPLETED: Step 1 - Configure standby-replay MDS ====")

        # Step 1.5: Mount filesystem
        ...
        try:
            ...
            log.warning("Failed to unmount fuse directory: %s" % e)
        try:
            if kernel_mount_dir:
            ...
            log.warning("Failed to unmount kernel directory: %s" % e)
