...
def test_cephfs_volume_creation(fs_util, client):
    ...
    for vol_name in shortest_valid_names:
        try:
            ...
            log.info(f"Test passed: Successfully created and validated volume '{vol_name}'.")
    ...
    try:
        ...
        longest_valid_names = [
            "x" * max_length,  # A name with maximum length consisting of a single repeated character
            ...
            "A" * (max_length // 2) + "_" * (max_length // 2),  # Alternating uppercase and underscores
        ]

        log.info(f"Testing longest valid volume names with lengths up to {max_length} characters.")

        for vol_name in longest_valid_names:
            try:
                log.info(f"Testing volume name of length {len(vol_name)}: '{vol_name[:10]}...'")
                fs_util.create_fs(client, vol_name, validate=True)
                log.info(f"Test passed: Successfully created and validated volume '{vol_name[:10]}...'.")
            except Exception as e:
                ...
                try:
                    ...
                    log.info(f"Cleanup not required: Volume '{vol_name[:10]}...' does not exist.")
    except Exception as overall_exception:
        log.error(f"Unexpected error in longest valid volume name test: {overall_exception}")

    # Test 3: Invalid volume names
    try:
        invalid_names = [
            "1", # Numeric 1 is invalid in CephFS
            "1" * max_length,  # A numeric name with the maximum length, numeric 1 is invalid in CephFS
            "",  # Empty name
            " ",  # Single space
            "   ",  # Multiple spaces
            "!@#$%^&*()",  # Special characters
        ...
        log.info(f"Testing invalid volume names.")

        for vol_name in invalid_names:
            try:
                log.info(f"Testing invalid volume name: '{vol_name}' (length {len(vol_name)})")
                fs_util.create_fs(client, vol_name, validate=False)
                log.error(f"Test failed: Volume with invalid name '{vol_name}' was created.")
            except CommandFailed:
    ...
    try:
        ...
        fs_util.create_fs(client, vol_name, validate=True)
        fs_util.create_fs(client, vol_name, validate=True)
        log.info(f"Test passed: Duplicate volume creation succeeded for '{vol_name}' (idempotent behavior).")
    except Exception as e:
    ...
    try:
        ...
        log.info(f"Testing invalid placement '{invalid_placement}' for volume '{vol_name}'")
        fs_util.create_fs(client, vol_name, placement=invalid_placement, validate=False)
        log.error(f"Test failed: Volume creation succeeded with invalid placement '{invalid_placement}'.")
    except CommandFailed:
        log.info(f"Test passed: Creation failed as expected for invalid placement '{invalid_placement}'.")
    except Exception as e:
...
def test_simultaneous_creation(fs_util, client):
    ...
    Cleanup:
        - Ensures all volumes are deleted at the end of the test, even if some creations fail or unexpected errors occur.
    ...
    try:
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(fs_util.create_fs, client, vol_name, validate=True): vol_name for vol_name in
                       vol_names}
        ...
        for vol_name in vol_names:
            try:
                ...
                log.warning(f"Cleanup not required: Volume '{vol_name}' does not exist.")
            except Exception as e:
...
def test_volume_deletion_with_mon_allow_pool_delete_false(fs_util, client):
    ...
    try:
        ...
        client.exec_command(sudo=True, cmd="ceph config set mon mon_allow_pool_delete false")
