...
| Client OS | RHEL 10.2 (RHEL 9 5.14+ backport also supported) |
| Mount type | Kernel (`mount -t ceph`) only |
| Out of scope | ODF, Kubernetes, CSI, `hostUsers: false` pod lifecycle |

## Test Plan

Full standalone test plan (v1.1, with developer review feedback incorporated):

**[CEPHFS_IDMAP_STANDALONE_TEST_PLAN.md](CEPHFS_IDMAP_STANDALONE_TEST_PLAN.md)**
...
Environment gating uses **feature presence** (sysfs `enable_unsafe_idmap` parameter and
functional idmap bind probe), not kernel version — RHEL 9 ships 5.14.x with the idmap
backport.
...
|--------|-----|-------------|----------|
| `test_idmap_preflight.py` | TC-S0 | Feature-based idmap gate, cluster health, plain mount smoke | P0 |
...
| `test_idmap_ownership.py` | TC-S3 | UID/GID translation + first/last/out-of-range boundaries | P0 |
...
| `test_idmap_acl.py` | TC-S7a | POSIX ACL UID mapping (`setfacl`/`getfacl`) | P1 |
...
| `test_idmap_unsafe_idmap.py` | TC-S14 | `enable_unsafe_idmap` fallback (conditional) | P2 |
...
├── README.md
├── CEPHFS_IDMAP_STANDALONE_TEST_PLAN.md
├── lib/
...
- RHEL 10.2 or RHEL 9 clients with `ceph-common` and kernel CephFS client
- `util-linux` with `X-mount.idmap` support
- `/sys/module/ceph/parameters/enable_unsafe_idmap` present (idmap backport probe)
- `acl` package for TC-S7a (`setfacl`/`getfacl`)
...
- `check_kernel_idmap_support()` — feature-based gate (sysfs + util-linux + probe)
- `mds_has_owner_uidgid()` — MDS feature detection for TC-S14 skip logic
...
3. TC-S7, TC-S7a, TC-S8–TC-S11, TC-S13 (P1)
4. TC-S14 (P2, conditional — skipped when MDS has `CEPHFS_FEATURE_HAS_OWNER_UIDGID`)

## References

- [RHSTOR-8028 / ODFRFE-119](https://bugzilla.redhat.com/) — User Namespace & CephFS Compatibility
- CephFS idmap: upstream ~6.7+, backported to RHEL 9 (5.14); gate on sysfs not version
- MDS `CEPHFS_FEATURE_HAS_OWNER_UIDGID` and `enable_unsafe_idmap` module parameter
