...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        helper.check_kernel_idmap_support(clients[0], plain_mount=plain_mount)
        helper.check_mds_high_uid_support(clients[0], plain_mount)
        mds_owner_uidgid = helper.mds_has_owner_uidgid(clients[0], plain_mount)
        log.info(
            "MDS CEPHFS_FEATURE_HAS_OWNER_UIDGID support: %s",
            mds_owner_uidgid,
        )
        ...
        if test_data is not None:
            test_data["idmap_suite"] = {
                ...
                "mds_has_owner_uidgid": mds_owner_uidgid,
            }
