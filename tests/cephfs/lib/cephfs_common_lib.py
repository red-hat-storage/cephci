...
class CephFSCommonUtils(FsUtils):
    ...
    def nfs_byok_test_setup(self, byok_setup_params):
        """
        ...
        ca_cert_alias = byok_setup_params["gklm_ca_cert_alias"]
        try:
            setup_gklm_infrastructure(
                nfs_nodes, gklm_ip, gklm_node_password, gklm_hostname, gklm_node_user
            )
            ...
            log.info(
                "Fetching RSA key and certificate from GKLM for NFS node credentials"
            )
            ...
            ca_cert = pre_requisite_for_gklm_get_ca(
                ...
                gklm_hostname=gklm_hostname,
                node_username=gklm_node_user,
                gklm_rest_client=gklm_rest_client,
                ca_cert_alias=ca_cert_alias,
            )
    ...
    def nfs_byok_test_cleanup(self, cleanup_params):
        """
        To Cleanup BYOK config on GKLM. Removes key object, GKLM client and cert-alias
        """
        gklm_rest_client = cleanup_params["gklm_rest_client"]
        gklm_client_name = cleanup_params["gklm_client_name"]
        gklm_cert_alias = cleanup_params["gklm_cert_alias"]
        # GKLM Symmetric Key Objects
        log.info("Retrieving GKLM symmetric key object IDs for cleanup")
        ids = [
            obj["uuid"]
            for obj in gklm_rest_client.objects.list_client_objects(gklm_client_name)
        ]
        log.info("Found %s symmetric key objects to remove", len(ids))
        ...
        gklm_rest_client.clients.delete_client(gklm_client_name)
        return 0
