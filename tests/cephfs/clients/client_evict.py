...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        rc = fs_util_v1.auto_evict(client1[0], client_info["clients"], 0)
        if rc == 0:
        ...
        ip_addr = fs_util.manual_evict(client1[0], 0)
        mds1_after_evict, _, rc = fs_util_v1.get_mds_info(
            *active_mds_list, info="session ls"
        )
        ...
        rc = fs_util_v1.config_blacklist_manual_evict(
            active_mds_node_obj_1, 0, service_name=active_mds_list[0]
        )
        if rc == 0:
            log.info("Configure blacklisting for manual evict success")
            rc = fs_util_v1.config_blacklist_manual_evict(
                active_mds_node_obj_1, 0, revert=True, service_name=active_mds_list[0]
            )
        else:
