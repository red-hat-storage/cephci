...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        log.info(
            "Peer status (initial): %s", json.dumps(status_before, indent=2)
        )
        ...
        for poll_i in range(SYNC_POLL_MAX_ITERATIONS):
            ...
            try:
                ...
                if syncing:
                    ...
                    log.info(
                        ...
                        poll_i, state, mode, eta,
                        crawl.get("state", ""), crawl.get("duration", ""),
                        dswait.get("state", ""), dswait.get("duration", ""),
                        read_tp, write_tp,
            ...
            raise CommandFailed(
                "S2: snap_full did not reach idle within %ds"
        ...
        log.info(
            ...
            sync_mode_full, eta_observed, crawl_observed,
            datasync_observed, throughput_observed,
        )
            ...
            try:
                ...
                if syncing and syncing.get("name") == "snap_delta":
                    ...
                    log.info(
                        ...
                        "sync_files=%s, total_files=%s",
                        poll_i, state, mode,
                        sync_bytes_str, sync_pct,
                        sync_files, total_files,
                    )
        ...
        log.info(
            "S3 Results: sync_mode_delta=%s, monotonic=%s, "
            "delta_polls_captured=%s",
            sync_mode_delta, monotonic, delta_poll_count,
        )
        ...
        log.info(
            "S4: path1 last_synced_snap: %s", json.dumps(path1_last, indent=2)
        )
        ...
        source_clients[0].exec_command(
            sudo=True, cmd="touch %sref_file" % mount_path1
        )
        source_clients[0].exec_command(
        ...
        if path1_synced < 1:
            raise CommandFailed(
                "snaps_synced should be >= 1 for %s, got %s"
                % (path1_key, path1_synced)
            )
