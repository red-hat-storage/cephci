...
def run(ceph_cluster, **kw):
    """
    CEPH- - Performs rsync vs mirroring tests: Mixed file sizes in KiB
    ...
    try:
        ...
        no_of_small_files = config.get("no_of_small_files", "20000")
        ...
        log.info("Adding data to subvolume %s: 1K to 64K file sizes", subvol)
        ...
        out, err = source_clients[0].exec_command(
            ...
            cmd=(
                ...
                f"--files {no_of_small_files} --file-size-distribution exponential "
                f"--files-per-dir {no_of_small_files} --dirs-per-dir 50 --top "
                f"{full_subvolume_path}/{mounting_dir}"
            ),
        )
        snapshot_name = f"snap_{mount_type}_1k_64k_sized_files"
