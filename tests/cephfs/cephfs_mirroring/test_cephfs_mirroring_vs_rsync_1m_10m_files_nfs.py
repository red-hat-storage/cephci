...
def run(ceph_cluster, **kw):
    """
    CEPH- - Performs rsync vs mirroring tests for 1M-10M mixed sized files with NFS mount.
    ...
    try:
        ...
        if subvol_1_nfs_export_name not in out:
            raise CommandFailed("Failed to create nfs export on source cluster")

        log.info("ceph nfs exports created successfully on source cluster")

        for nfs_export_name in [subvol_1_nfs_export_name]:
            out, rc = source_clients[0].exec_command(
        ...
        if not rc:
            ...
            return 1
        log.info(
            "Adding data to subvol_1: Multiple small files of size range 1M to 10M"
        )
        cmd = f"""
        mkdir -p {nfs_mounting_dir_1}{mounting_dir}
        bash -c '
        target=$((5*1024))
        written=0
        idx=1
        while [ $written -lt $target ]; do
            size=$((RANDOM % 10 + 1))
            dd if=/dev/zero of="{nfs_mounting_dir_1}{mounting_dir}/file_${{idx}}_${{size}}M.bin" bs=1M count=$size
        ...
        snapshots = ["snap_n1_1m_to_10m_files"]
        source_clients[0].exec_command(
            sudo=True,
        ...
        small_1m_to_10m_n1 = f"{target_remote_nfs}/small_1m_to_10m_n1"
        small_1m_to_10m_n2_snap = f"{target_remote_nfs}/small_1m_to_10m_n2_snap"
        ...
        for dir1 in [small_1m_to_10m_n1, small_1m_to_10m_n2_snap]:
        ...
        out, err = source_clients[0].exec_command(
            ...
            cmd=fs_mirroring_utils.get_rsync_command(
                ...
                target_path=small_1m_to_10m_n1,
        ...
        out, err = source_clients[0].exec_command(
            ...
            cmd=fs_mirroring_utils.get_rsync_command(
                ...
                target_path=small_1m_to_10m_n2_snap,
                target_ip=target_clients[0].node.ip_address,
            ),
        )
        log.info(out)
        ...
        if config.get("cleanup", True):
            ...
            paths_to_unmount = [nfs_mounting_dir_1]
