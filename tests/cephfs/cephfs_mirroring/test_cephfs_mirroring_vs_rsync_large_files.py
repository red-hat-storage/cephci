...
def run(ceph_cluster, **kw):
    """
    CEPH- - Performs rsync vs mirroring tests: Single Large Files
    ...
    try:
        ...
        file_block_counts = config.get("file_block_size", "5120")
        no_of_large_files = config.get("no_of_large_files", "1")
        ...
        file_name = f"hello_{mount_type}"

        log.info("Adding data to %s: %s large file", subvol, no_of_large_files)
