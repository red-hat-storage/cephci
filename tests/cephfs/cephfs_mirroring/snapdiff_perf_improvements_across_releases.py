...
csv_file = f"{log_dir}/snapshot_sync_info.csv"
...
def log_snapshot_info(snapshot_type, snapshot_info):
...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        source_clients[0].exec_command(
            sudo=True,
            cmd="dnf clean all ; dnf update -y --nogpgcheck",
        )
        ...
        ceph_version_out = ceph_version_cmd[
            0
        ].strip()  # Access the first element of the tuple
        ...
        source_fs = config.get("source_fs")
        ...
        for path in file_paths:
            create_large_files(source_clients[0], path, 500, 20)
        ...
        if initial_snapshot_k_info:
            ...
            log_snapshot_info("Kernel Initial", initial_snapshot_k_info)
        if initial_snapshot_f_info:
            ...
            log_snapshot_info("Fuse Initial", initial_snapshot_f_info)
        ...
        if incr1_snapshot_k_info:
            ...
            log_snapshot_info("Kernel Incremental 1", incr1_snapshot_k_info)
        if incr1_snapshot_f_info:
            ...
            log_snapshot_info("Fuse Incremental 1", incr1_snapshot_f_info)
        ...
        if incr2_snapshot_k_info:
            ...
            log_snapshot_info("Kernel Incremental 2", incr2_snapshot_k_info)
        if incr2_snapshot_f_info:
            ...
            log_snapshot_info("Fuse Incremental 2", incr2_snapshot_f_info)
        ...
        if incr3_snapshot_k_info:
            ...
            log_snapshot_info("Kernel Incremental 3", incr3_snapshot_k_info)
        if incr3_snapshot_f_info:
            ...
            log_snapshot_info("Fuse Incremental 3", incr3_snapshot_f_info)
