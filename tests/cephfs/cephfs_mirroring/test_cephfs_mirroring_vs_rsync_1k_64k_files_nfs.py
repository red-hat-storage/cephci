...
def run(ceph_cluster, **kw):
    """
        CEPH- - Performs rsync vs mirroring tests for 1K-64K mixed sized files with NFS mount.
    .
    ...
    try:
        ...
        if not rc:
            ...
            return 1
        log.info(
            "Adding data to subvol_1: Multiple small files of size range 1k to 64k"
        )
        out, err = source_clients[0].exec_command(
            ...
            f"{nfs_mounting_dir_1}{mounting_dir}",
        )
        log.info(err)
        log.info(out)

        log.info("Creating snapshots")
        snapshots = ["snap_n1_1k_to_64k_files"]
        source_clients[0].exec_command(
            sudo=True,
        ...
        small_1k_to_64k_n1 = f"{target_remote_nfs}/small_1k_to_64k_n1"
        small_1k_to_64k_n2_snap = f"{target_remote_nfs}/small_1k_to_64k_n2_snap"
        ...
        for dir1 in [small_1k_to_64k_n1, small_1k_to_64k_n2_snap]:
        ...
        out, err = source_clients[0].exec_command(
            ...
            cmd=fs_mirroring_utils.get_rsync_command(
                ...
                target_path=small_1k_to_64k_n1,
        ...
        out, err = source_clients[0].exec_command(
            ...
            cmd=fs_mirroring_utils.get_rsync_command(
                ...
                target_path=small_1k_to_64k_n2_snap,
                target_ip=target_clients[0].node.ip_address,
            ),
        )
        log.info(err)
