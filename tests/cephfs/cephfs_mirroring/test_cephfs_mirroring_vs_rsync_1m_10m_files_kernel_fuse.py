...
def run(ceph_cluster, **kw):
    """
    CEPH- - Performs rsync vs mirroring tests for 1M-10M mixed sized files with Kernel and Fuse mount.
    ...
    try:
        ...
        target_clients = ceph_cluster_dict.get("ceph2kdhadu").get_ceph_objects("client")
        ...
        cmd = f"""
        mkdir -p {dir_struct['subvol_1']}{mounting_dir}
        bash -c '
        target=$((5*1024))
        written=0
        idx=1
        while [ $written -lt $target ]; do
            size=$((RANDOM % 10 + 1))
            dd if=/dev/zero of="{dir_struct['subvol_1']}{mounting_dir}/file_${{idx}}_${{size}}M.bin" bs=1M count=$size
        ...
        log.info(
            "Adding data to subvol_2: Multiple small files of size range 1M to 10M"
        )
        cmd = f"""
        mkdir -p {dir_struct['subvol_2']}{mounting_dir}
        bash -c '
        target=$((5*1024))
        written=0
        idx=1
        while [ $written -lt $target ]; do
            size=$((RANDOM % 10 + 1))
            dd if=/dev/zero of="{dir_struct['subvol_2']}{mounting_dir}/file_${{idx}}_${{size}}M.bin" bs=1M count=$size
            written=$((written + size))
            idx=$((idx + 1))
        done
        '
        ...
        snapshots = ["snap_k1_1m_to_10m_files", "snap_f1_1m_to_10m_files"]
        ...
        small_1m_to_10m_k1 = f"{target_remote_kernel}/small_1m_to_10m_k1"
        small_1m_to_10m_f1 = f"{target_remote_fuse}/small_1m_to_10m_f1"
        small_1m_to_10m_k2_snap = f"{target_remote_kernel}/small_1m_to_10m_k2_snap"
        small_1m_to_10m_f2_snap = f"{target_remote_fuse}/small_1m_to_10m_f2_snap"
        ...
        for dir1 in [
            small_1m_to_10m_k1,
            small_1m_to_10m_f1,
            small_1m_to_10m_k2_snap,
            small_1m_to_10m_f2_snap,
        ]:
        ...
        out, err = source_clients[0].exec_command(
            ...
            cmd=fs_mirroring_utils.get_rsync_command(
                ...
                target_path=small_1m_to_10m_k1,
        ...
        out, err = source_clients[0].exec_command(
            ...
            cmd=fs_mirroring_utils.get_rsync_command(
                ...
                target_path=small_1m_to_10m_f1,
                target_ip=target_clients[0].node.ip_address,
            ),
        )
        log.info(out)
        ...
        out, err = source_clients[0].exec_command(
            ...
            cmd=fs_mirroring_utils.get_rsync_command(
                ...
                target_path=small_1m_to_10m_k2_snap,
                target_ip=target_clients[0].node.ip_address,
            ),
        )
        log.info(out)
        ...
        out, err = source_clients[0].exec_command(
            ...
            cmd=fs_mirroring_utils.get_rsync_command(
                ...
                target_path=small_1m_to_10m_f2_snap,
                target_ip=target_clients[0].node.ip_address,
            ),
        )
        log.info(out)
