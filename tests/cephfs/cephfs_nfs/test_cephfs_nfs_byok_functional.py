...
from tests.nfs.byok.byok_tools import get_enctag, load_gklm_config
...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        enctag = None
        byok_status, enctag, gklm_rest_client, cert = (
            cephfs_common_utils.nfs_byok_test_setup(byok_setup_params)
        )
        if byok_status:
        ...
        if cleanup:
            try:
                if enctag is not None:
                    cephfs_common_utils.nfs_byok_test_cleanup(byok_setup_params)
                if byok_test_params is not None:
                    if byok_test_params.get("gklm_client_name_1"):
                        cleanup_params = {
                            ...
                            "gklm_client_name": byok_test_params["gklm_client_name_1"],
                            "gklm_cert_alias": byok_setup_params["gklm_cert_alias_1"],
                        }
                        cephfs_common_utils.nfs_byok_test_cleanup(cleanup_params)
