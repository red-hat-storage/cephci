...
def run(ceph_cluster, **kw):
    ...
    4. Run some IO's
    6. while the IO, add host in ingress.yaml
    7. Apply it while IO's are running
    ...
    try:
        ...
        ingress = {
            ...
            "placement": {
                "hosts": [
                    nfs_list[0].node.hostname,
                    nfs_list[1].node.hostname,
                ],  # add nfs_list[2].node.hostname
                "count": 1,
            },
            "spec": {
        ...
        client1.exec_command(
            sudo=True, cmd=f"echo '{ingress_updated}' > /tmp/ingress_updated.yaml"
        )
        with parallel() as p:
            p.spawn(
            ...
            p.spawn(
                client1.exec_command,
                sudo=True,
                cmd="ceph orch apply -i /tmp/ingress_updated.yaml",
            )

        # validate_service
        fs_util.validate_services(client1, f"ingress.nfs.{nfs_ID}")
        # run IOs again and check no parrallel
        dir_name2 = "smallfile_dir_2"
        client1.exec_command(sudo=True, cmd=f"mkdir -p {nfs_mounting_dir}{dir_name2}")
        smallfile(client1, nfs_mounting_dir, dir_name2)
