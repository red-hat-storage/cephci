    ...
    * Export a file with specific permissions and confirm that the correct permissions are applied to the
    ...
    try:
        ...
        if compare_file_attributes(src_nfs_file_attributes, des_nfs_file_attributes):
            log.info(
                "File Attributes on a NFS File are Retained after copying to local filesystem"
            )
        else:
            log.info(
        ...
        if compare_file_attributes(
            src_local_file_attributes, des_local_file_attributes
        ):
            log.info(
                "File Attributes on a Local File are Retained after copying to NFS mount"
            )
        else:
            log.info(
        ...
        def unmount_directory(client, directory_path):
            client.exec_command(
                sudo=True, cmd=f"umount -l {directory_path}", check_ec=False
            )
