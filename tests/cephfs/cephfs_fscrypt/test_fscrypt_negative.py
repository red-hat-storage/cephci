...
def fscrypt_filename_length(fscrypt_test_params):
    ...
    if fscrypt_util.lock(mnt_client, encrypt_path):
        return 1
    if verify_encryption(fscrypt_util, mnt_client, encrypt_path, file_list):
        return 1
    return 0
...
def fscrypt_key_remove_diff_clients(fscrypt_test_params):
    ...
    if verify_encryption(fscrypt_util, mnt_client_2, encrypt_path_2, file_list_1):
        return 1
    log.info(
    ...
    if not verify_encryption(fscrypt_util, mnt_client_2, encrypt_path_2, file_list_2):
    ...
    if exp_str not in str(out):
        ...
        return 1
    if verify_encryption(fscrypt_util, mnt_client_2, encrypt_path_2, file_list_2):
        log.error("Files are not locked in client2 after key removal")
        return 1
    time.sleep(5)
    if not verify_encryption(fscrypt_util, mnt_client_1, encrypt_path_1, file_list_1):
...
def is_encrypted(file_path, old_file_path, mnt_client):
    """
