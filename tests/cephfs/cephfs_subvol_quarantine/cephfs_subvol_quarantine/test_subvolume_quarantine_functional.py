...
No workarounds for regressions against the MGR contract:
  ...
  - info enabled (minimal: name/group/quarantine status on quarantined SVs)
...
def f13_logs_set_unset(qtn, fs_util, admin, fuse, vol, config):
    ...
    try:
        ...
        mgr_node.exec_command(
            sudo=True,
            cmd=f"cephadm logs --name {daemon_name} > /tmp/qtn-f13-mgr.log",
            check_ec=False,
        )
...
def f14_mgr_ops_matrix(qtn, fs_util, admin, fuse, vol, config):
    ...
    Blocked: resize, rm, snapshot create, clone, getpath
    Allowed: ls (names only), info (minimal quarantine status),
             snapshot ls, earmark get/set
    """
    sub_a = _cfg(config, "sub_a", "qtn-sv-a")
    sub_b = _cfg(config, "sub_b", "qtn-sv-b")
    try:
        ...
        log.info("--- Expect SUCCEED (ls / minimal info / metadata) ---")
