...
Acceptance TCs (primary):
  acceptance1_quarantine_flag   — enable succeeds; info/ls reflect quarantine (hard fail on bugs)
...
def subtest_acceptance1_quarantine_flag(qtn, admin, fuse_client, vol_name, config):
    ...
    Expected (no workarounds):
      - enable returns status successful
      - subvolume ls still lists the subvolume
      - subvolume info succeeds and shows quarantine/quarantined field
    """
    ...
    try:
        ...
        log.info("Re-check: subvolume ls must still list %s (no workaround)", lab.sub_a)
        if qtn.assert_subvolume_ls_complete(admin, vol_name, [lab.sub_a]):
            log.error(
                "Acceptance 1 FAILED: subvolume ls incomplete while quarantined "
                "(reported product issue — failing hard)"
            )
            return 1

        log.info("Re-check: subvolume info must show quarantine state")
...
def subtest_isolation_sibling(qtn, admin, fuse_client, vol_name, config):
    ...
    try:
        ...
        # ls must still list both (hard fail if incomplete — reported issue)
        if qtn.assert_subvolume_ls_complete(admin, vol_name, [lab.sub_a, lab.sub_b]):
            log.error(
                "isolation_sibling: subvolume ls incomplete while quarantined "
                "(reported product issue — failing hard)"
            )
            return 1
...
def subtest_e2e_incident_workflow(qtn, admin, fuse_client, vol_name, config):
    ...
    try:
        ...
        log.info("E2E-5: subvolume info shows quarantined state (hard assert)")
        if qtn.assert_info_quarantined(admin, vol_name, lab.sub_a):
            return 1

        log.info("E2E-5b: subvolume ls still complete (hard assert)")
        if qtn.assert_subvolume_ls_complete(admin, vol_name, [lab.sub_a, lab.sub_b]):
            return 1
