...
def run(ceph_cluster, **kw):
    try:
        ...
        dirs, rc = fs_util.mkdir(client1, 0, 4, client_info["mounting_dir"], dir_name)
        if rc == 0:
        ...
        with parallel() as p:
            ...
            for node in client_info["mds_nodes"]:
                p.spawn(fs_util.daemon_systemctl, node, "mds", "active_mds_restart")
        with parallel() as p:
            for node in client_info["mds_nodes"]:
        ...
        fs_util.standby_rank(
            client_info["mds_nodes"], client_info["mon_node"], todo="add_rank_revert"
        )
