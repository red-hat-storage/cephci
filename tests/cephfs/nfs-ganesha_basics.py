...
def run(ceph_cluster, **kw):
    try:
        ...
        if "5." in rhbuild:
            ...
            nfs_name = "cephfs-nfs"
            out, rc = nfs_client[0].exec_command(cmd=" sudo ceph fs ls | awk {' print $2'} ")
            ...
            out, rc = nfs_client[0].exec_command(sudo=True, cmd=f"ceph nfs cluster create {nfs_name} {nfs_server_name}")
            ...
            nfs_client[0].exec_command(sudo=True, cmd=f"ceph nfs export create cephfs {fs_name} {nfs_name} "
                                                 f"{nfs_export_name} path={path}"
            )
            # Verify ceph nfs export is created
            out, rc = nfs_client[0].exec_command(sudo=True, cmd=f"ceph nfs export ls {nfs_name}")
            output = out.read().decode()
            output.split()
            ...
            # Delete nfs cluster
            nfs_client[0].exec_command(sudo=True, cmd=f"ceph nfs cluster delete {nfs_name}")
            ...
            nfs_client[0].exec_command(cmd=f"sudo rm -rf {mounting_dir}*")
            log.info("Unmounting nfs-ganesha mount on client:")
            nfs_client[0].exec_command(cmd="sudo umount %s -l" % (nfs_mounting_dir))
            log.info("Removing nfs-ganesha mount dir on client:")
            nfs_client[0].exec_command(cmd="sudo rm -rf  %s" % (nfs_mounting_dir))
