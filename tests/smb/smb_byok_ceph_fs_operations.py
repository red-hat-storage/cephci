...
def run(ceph_cluster, **kw):
    ...
    sub_dir = config.get("sub_dir", "/volumes/smb-byok-grp/svol1")
    ...
    try:
        if byok_operation == "test_ceph_fs_mount":
            ...
            rc, err = client.exec_command(
                sudo=True,
                cmd=f"dir=$(ls {mount_point}/); dd if=/dev/zero of={mount_point}/$dir/ceph_file1 bs=600M count=1",
                check_ec=False,
            )
            if "Required key not available" not in err:
                raise OperationFailedError(
                    "Write operation should fail with 'Required key not available', but did not fail."
                )

        elif byok_operation == "test_file_ops_ceph_fs_mount":
            rc, err = client.exec_command(
                sudo=True,
                cmd=f"cd {mount_point}/*/ && mkdir test",
                check_ec=False,
            )
            if "Required key not available" not in err:
                raise OperationFailedError(
        ...
        log.error(f"Failed to perform byok cephfs operations {byok_operation} : {e}")
        return 1
    finally:
        if mount_cleanup:
            client.exec_command(
