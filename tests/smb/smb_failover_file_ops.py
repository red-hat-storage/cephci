...
def client_operations(clients, operations, mount_point, file_count, windows_client):
    ...
    try:
        ...
        raise OperationFailedError(f"Samba client operations failed, Error {e}")


def run(ceph_cluster, **kw):
    """Samba failover with IOs
    Args:
    ...
    # GetFailover type
    failover_type = config.get("failover_type", "reboot")
