...
def run(ceph_cluster, **kw):
    ...
    mon_short_name_list = [
        ceph_node.shortname for ceph_node in ceph_cluster.get_nodes("mon")
    ]
    ...
    for _mon_to_kill in mon_to_kill_list:
        ...
        if len(matched_short_names) > 0:
            shrinked_nodes = [
                ...
                if ceph_node.shortname in matched_short_names
            ]
            for ceph_node in shrinked_nodes:
                ceph_node.remove_ceph_object(ceph_node.get_ceph_objects("mon")[0])
        else:
            raise RuntimeError(
                "No match for {node_name}".format(node_name=_mon_to_kill)
            )
        mon_to_kill = (
            ",".join([mon_to_kill, ",".join(matched_short_names)])
            if mon_to_kill
            else ",".join(matched_short_names)
        )
    ...
    if build.startswith("4"):
        ...
        ceph_installer.exec_command(
            sudo=True,
            cmd="cd {ansible_dir};cp -R {ansible_dir}/infrastructure-playbooks/{playbook} .".format(
