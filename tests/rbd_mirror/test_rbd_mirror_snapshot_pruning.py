...
def test_unsynced_failover_snapshot_pruning(
    ...
    **kw,
    ...
    pre_stop_snaps = get_mirror_snapshots(rbd_secondary, pool, image)
    log.info(
        f"[Flow B] Cluster-2 synced. Non-primary snaps: "
        f"{[s['name'] for s in pre_stop_snaps if is_mirror_non_primary_snap(s)]}"
    )
    ...
    log.info(
        "[Flow B] Step 20: Writing 1 GiB and creating new snapshot on Cluster-1 (unsynced)"
    )
    out, err = rbd_primary.bench(
        **{
            "image-spec": imagespec,
    ...
    if err:
        raise Exception(f"rbd bench write failed for {imagespec}: {err}")
    rbd_primary.mirror.image.snapshot(**{"image-spec": imagespec})

    # ── Step 21: Verify Cluster-2 snaps are unchanged ───────────────────────
    log.info("[Flow B] Step 21: Verifying Cluster-2 snaps unchanged (daemon stopped)")
    post_stop_snaps = get_mirror_snapshots(rbd_secondary, pool, image)
