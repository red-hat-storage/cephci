    ...
    for pool_type in pool_types:
        ...
        for pool, pool_config in multi_pool_config.items():
            ...
            for image, image_config_val in image_config.items():
                ...
                pri_image_spec = (
                    f"{pool}/{namespace}/{image}" if namespace else f"{pool}/{image}"
                )
                sec_image_spec = (
                    f"{pool}/{remote_namespace}/{image}"
                    if remote_namespace
                    else f"{pool}/{image}"
                )
                # Enable snapshot mode mirroring on images of the namespace
                ...
                if image_config_val.get(
                    ...
                    for level, interval in zip(
                        ...
                        # Adding snapshot schedules to the mirrored images
                        out, err = add_snapshot_scheduling(
                            rbd_primary, **snap_schedule_config
                        )
                        if err:
                    ...
                    # Prepare arguments based on level
                    verify_args = {
                        "rbd": rbd_primary,
                        "pool": pool,
                        "interval": interval,
                    ...
                    if level == "namespace":
                        ...
                        verify_args["namespace"] = namespace  # Will be None if not used
                        verify_args["image"] = image

                    # Perform snapshot schedule verification
                    if verify_snapshot_schedule(**verify_args):
                        raise Exception(
                            f"Snapshot schedule verification failed at {level} level with interval: {interval}"
                        )
            ...
            # Log test success with config details
            namespace_mirror_type = (
                kw.get("config", {}).get(pool_type, {}).get("namespace_mirror_type")
            )
            snap_schedule_levels = image_config_val.get("snap_schedule_levels")
            log.info(
                f"Test passed for pool_type: {pool_type}, "
                f"namespace_mirror_type: {namespace_mirror_type}, "
                f"snap_schedule_levels: {snap_schedule_levels}"
            )
    return 0
