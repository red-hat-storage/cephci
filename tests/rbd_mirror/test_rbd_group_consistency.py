...
Step 16: Wait for the md5sum of the smaller image on site-B to be same as that of in step 6.
Step 17: Force promote on site-B while the larger image in the group snapshot is still mirroring.
step 18: Get the md5sum of small and large image on siteB
step 19: Repeat above on EC pool with or without namespace.
Step 20: Cleanup rbd test objects like pool, images, groups etc
"""
    ...
    **kw,
    ...
    for pool_type in pool_types:
        ...
        for pool, pool_config in multi_pool_config.items():
            ...
            bench_kw.update(
                {
                    "io-total": kw.get("config", {})
                    .get("io_small", {})
                    .get("io-size_init", "20M")
                }
            )
            bench_kw.update(
                {
                    "io-threads": kw.get("config", {})
                    .get("io_small", {})
                    .get("io-threads", "16")
                }
            )
            ...
            # bench_kw = kw.get("config", {}).get("io_small", {}).get("io-size", "40M")
            for image_spec in image_spec_small:
                bench_kw.update({"image-spec": image_spec})
                out, err = rbd_primary.bench(**bench_kw)
...
def run(**kw):
    ...
    try:
        ...
        log.info(
            "Test Verify group level data consistency on secondary after primary site disaster passed"
        )
    except Exception as e:
        log.error(
            "Test Verify group level data consistency on secondary after primary site disaster failed: "
            + str(e)
        )
        return 1
