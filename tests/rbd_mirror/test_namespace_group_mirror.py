    ...
    for pool_type in pool_types:
        ...
        for pool, pool_config in multi_pool_config.items():
            ...
            while retry < 10:
                ...
                if err:
                    raise Exception(
                        "Failed to get image status on standalone image: "
                        + group_config["pool"]
                        + "/"
                        + pool_config.get("namespace")
                        + "/"
    ...
    for pool_type in pool_types:
        ...
        for pool, pool_config in multi_pool_config.items():
            ...
            out, err = rbd_primary.group.rename(
                **{
                    "source-group-spec": pool
                    + "/"
                    + group_config["namespace"]
                    + "/"
                    + group_config["group"],
                    "dest-group-spec": pool
                    + "/"
                    + group_config["namespace"]
                    + "/"
                    + group_new,
            ...
            group_config.update({"group": group_new})
            ...
            service_name = mirror1.get_rbd_service_name("rbd-mirror")
            mirror1.change_service_state(service_name=service_name, operation="stop")
            time.sleep(5)
            group_mirror_status_verify(
                primary_cluster,
                secondary_cluster,
                rbd_primary,
                rbd_secondary,
                primary_state="down+stopped",
                secondary_state="up+replaying",
                **group_config
            )
            log.info("Successfully verified group status changed to down+stopped")
            ...
            mirror1.change_service_state(service_name=service_name, operation="start")
            time.sleep(5)
            group_mirror_status_verify(
                primary_cluster,
                secondary_cluster,
            ...
            log.info("Successfully verified group status changed to up+stopped")
