...
def run(ceph_cluster, **kw):
    ...
    while time.time() - start_time < duration:
        ...
        if new_active_mgr is not active_mgr:
            ...
            if status:
                break
        log.warning(
            "Either new active MGR is unavailable or all the MGRs are yet to be added back"
        )
        time.sleep(30)
    ...
    while time.time() - start_time < duration:
        ...
        if status:
            break
        log.warning("All the MGRs are yet to be added back")
        time.sleep(30)
    ...
    log.info("Scenario 3: Verification of adding and removing mgr daemon using labels")
    ...
    log.info("Scenario 4: Verification of scaleup and down of mgr daemon using count")
    ...
    result = mgr_obj.mgr_scale_up_down_bylist(mgr_nodes)
    if not result:
    ...
    mgr_nodes.pop()
    # scale down scenario
    mgr_obj.mgr_scale_up_down_bylist(mgr_nodes)
    mgr_nodes = rados_obj.get_daemon_list_fromCluster(daemon_type="mgr")
    mgr_count = len(mgr_nodes)
    if not result:
