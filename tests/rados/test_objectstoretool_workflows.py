...
def run(ceph_cluster, **kw):
    ...
    objectstore_enospc_obj = objectstoreToolWorkflows(node=cephadm, nostart=True, nostop=True)
        ...
        if config.get("bluestore-enospc"):
            ...
            for operation in operations:
                ...
                if operation == "list_pgs":
                    ...
                    pg_list = [json.loads(x) for x in out.split()]
                ...
                if operation == "get_omap":
                    ...
                    out = objectstore_enospc_obj.list_omap(osd_id=_osd_id, pgid=omap_pg, obj=omap_obj)
                    ...
                    log.info(f"Value for OMAP key {omap_key} in object {omap_obj}: {value}")
                ...
                if operation == "get_omap_header":
                    ...
                    objectstore_enospc_obj.get_omap_header(
                        osd_id=_osd_id, obj=omap_obj, pgid=omap_pg, out_file="/tmp/omaphdr_org"
                    )
                ...
                if operation == "get_bytes":
                    ...
                    log.info(f"Fetch object bytes for obj: {bench_obj} in PG: {bench_pg}")
                    objectstore_enospc_obj.get_bytes(
                        osd_id=_osd_id, obj=bench_obj, pgid=bench_pg, out_file="/tmp/obj_work"
                    )
                    ...
                    objectstore_enospc_obj.get_bytes(
                        osd_id=_osd_id, obj=bench_obj, pgid=bench_pg, out_file="/tmp/mod_obj"
                    )
                ...
                if operation == "dump":
                    ...
                    out = objectstore_enospc_obj.fetch_object_dump(osd_id=_osd_id, pgid=bench_pg, obj=bench_obj)
                    log.info(out)
