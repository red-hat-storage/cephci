...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        if config.get("bluestore-enospc"):
            ...
            # wait for 300 secs to see ENOSPC crash in OSD log
            while datetime.datetime.now() < timeout_time:
                for _osd_id in acting_set:
                    if "down" == rados_obj.fetch_osd_status(_osd_id):
                        ...
                        break
                log.warning(
                    "OSDs % are up and running, re-check after 30 secs" % acting_set
                )
            else:
                log.warning("None of the OSDs went down with ENOPSC, rerun rados bench")
                rados_obj.bench_write(
                    pool_name=_pool_name,
                    rados_write_duration=500,
                    max_objs=objs_enospc,
            ...
            for operation in operations:
                ...
                if operation == "list_objects":
                    ...
                    out = objectstore_obj.get_pg_from_object(
                        osd_id=_osd_id, obj_id=object_id
                    )
                    ...
                    log.info(
                        f"\n ----------------------------------"
                        f"\n Identify all objects within a placement group {pg_id} for OSD.{_osd_id}"
                        f"\n ----------------------------------"
                    )
                    out = objectstore_obj.list_objects(osd_id=_osd_id, pgid=pg_id)
                    log.info(
                        f"List of objects in OSD.{_osd_id} and PG {pg_id}: \n {out}"
                    )
                ...
                if operation == "list_get_omap":
                    ...
                    pg_id, obj = get_omap_obj(_osd_id)
                    log.info(f"PG: {pg_id}")
                    ...
                    out = objectstore_obj.list_omap(osd_id=_osd_id, pgid=pg_id, obj=obj)
                    ...
                    value = objectstore_obj.get_omap(
                        osd_id=_osd_id, obj=obj, pgid=pg_id, key=omap_key
                    )
                ...
                if operation == "get_omap_header":
                    ...
                    log.info(
                        f"\n ------------------------------------------------------"
                        f"\n Fetch OMAP header for object {obj} in OSD {_osd_id} and PG: {pg_id}"
                        f"\n ------------------------------------------------------"
                    )
                    ...
                    objectstore_obj.get_omap_header(
                        osd_id=_osd_id, obj=obj, pgid=pg_id, out_file="/tmp/omaphdr_org"
                    )
                ...
                if operation == "list_get_attributes":
                    ...
                    out = objectstore_obj.list_attributes(
                        osd_id=_osd_id, pgid=pg_id, obj=obj
                    )
                    attr_list = out.split()
                    ...
                    value = objectstore_obj.get_attribute(
                        osd_id=_osd_id, obj=obj, pgid=pg_id, attr=obj_attr
                    )
                    log.info(
                ...
                if operation == "get_bytes":
                    ...
                    objectstore_obj.get_bytes(
                        osd_id=_osd_id, obj=obj, pgid=pg_id, out_file="/tmp/obj_work"
                    )
                    ...
                    objectstore_obj.get_bytes(
                        osd_id=_osd_id, obj=obj, pgid=pg_id, out_file="/tmp/mod_obj"
                    )
                ...
                if operation == "dump":
                    ...
                    pg_id, obj = get_bench_obj(_osd_id)
                    out = objectstore_obj.fetch_object_dump(osd_id=_osd_id, obj=obj)
                    log.info(out)
                ...
                if operation == "export":
                    ...
                    out = objectstore_obj.export(
                        osd_id=_osd_id, pgid=pg_id, obj_name=obj
                    )
                    log.info(out)
                ...
                if operation == "get-superblock":
                    ...
                    out = objectstore_obj.get_superblock(osd_id=_osd_id)
                    log.info(out)
        else:
