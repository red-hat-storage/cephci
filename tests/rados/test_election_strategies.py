    ...
    if config.get("stretch_mode"):
        try:
            ...
            check_rank_after_mon_removal(
                mon_obj=mon_obj, mon_workflow_obj=mon_workflow_obj, rados_obj=rados_obj
            )
...
def add_all_mons_to_disallow_list(mon_obj) -> None:
    ...
    try:
        ...
        expected_error_msg = f"Error EINVAL: mon.{last_allowed_mon} is the only remaining allowed leader!"
        generated_error_msg = e.args[0].split("\n")[4]
        if generated_error_msg != expected_error_msg:
            raise AssertionError(
                f"Generated Error: {generated_error_msg}"
                f"Expected Error: {expected_error_msg}"
            )
        log.info(
...
def check_rank_after_mon_removal(mon_obj, mon_workflow_obj, rados_obj):
    ...
    - Add back removeed Monitor daemon
    ...
    def check_rank_change(
        ...
        for mon_name, mon_rank in quorum_before_mon_removal.items():
            if mon_rank == removed_mon_rank:
                ...
                quorum_before_mon_removal[mon_name] = (
    ...
    for index in [0, n // 2, -1]:
        ...
        log.info(
            f"Validating rank updation when Mon name: {mon_name} rank: {mon_rank} is removed and added back"
        )
...
def disallowed_leader_checks(mon_obj, election_strategy: str) -> bool:
    ...
    if lowest_ranked_mon in disallowed_list:
        log.error(
            "Mon with lowest rank is still in disalloweded leaders list post removal"
        )
        return False
