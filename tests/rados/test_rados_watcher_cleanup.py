...
def run(ceph_cluster, **kw):
    ...
    try:
        log.info("Creating test pool: ", pool)
        ...
        if not rados_obj.create_pool(pool_name=pool, app_name="rbd"):
            raise Exception("Failed to create and initialize pool", pool)
        ...
        if client_nodes:
            ...
            if blacklist_image_name and blacklist_device:
                try:
                    ...
                    log.info(
                        "Cleanup method 'blacklist' verified successfully for ",
                        blacklist_image_name,
                    )
                    ...
                    log.info(
                        "Ensuring clean state after 'blacklist' test for ",
                        blacklist_image_name,
                    )
                    ...
                    try:
                        ...
                        if rc != 0 and "No such entry" not in err:
                            ...
                            raise Exception(
                                "Blacklist removal failed with error: ", err
                            )
                        log.info(
...
def _verify_watcher_cleanup(
    ...
    for attempt in range(max_retries):
        ...
        if client_ip:
            ...
            if not watchers:
                log.info("All watchers successfully cleaned up for ", image_name)
                watcher_cleaned = True
                break
            else:
                log.warning(
    ...
    if not watcher_cleaned and method.lower() == "umount":
        ...
        for line in out.strip().splitlines():
            ...
            if len(fields) >= 5 and fields[2] == image_name:
                rbd_dev = fields[4]
                log.info("Attempting fallback rbd unmap on device: ", rbd_dev)
                try:
        ...
        if client_ip and client_ip not in watchers:
            log.info("Watcher removed via fallback 'rbd unmap' for ", client_ip)
            return
...
def _delete_watcher_and_verify(
    ...
    if method == "unmap":
        log.info("Performing rbd unmap of device ", device)
        ...
        log.info("Unmounting ", mount_path)
        _, err = client.exec_command(
        ...
        try:
            ...
            if any(
                ...
                log.info("Expected disconnect after reboot: ", str(e))
            else:
                raise Exception("Unexpected error during reboot: ", str(e))
