...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        if "scenario-5" in scenarios_to_run:
            log.info(
                "STARTING :: Scenario 5 -> Tests for inbetween overwrite with varying write_size"
            )
            ...
            for recompression_min_gain in recompression_min_gain_to_test:
                for object_size in object_sizes_to_test:
                    ...
                    bluestore_compression.inbetween_overwrite(**kwargs)

            log.info(
                "COMPLETED :: Scenario 5 -> Tests for inbetween overwrite with varying write_size"
            )
        ...
        if "scenario-7" in scenarios_to_run:
            log.info("STARTED: Scenario 7: Disable bluestore_write_v2 and validate")
            bluestore_compression.toggle_bluestore_write_v2(toggle_value="false")
            log.info("COMPELTED: Scenario 7: Disable bluestore_write_v2 and validate")
