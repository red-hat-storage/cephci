...
def run(ceph_cluster, **kw):
    ...
    Scenarios (config.scenarios) and test steps:
        ...
        scenario7: Rack FD with max=100. Step 1: Create CRUSH rule and pool. Steps 2-5:
            ok-to-upgrade (max=100) for osd, host, rack, chassis and verify.
        ...
        if "scenario1" in scenarios:
            ...
            try:
                for convergence_factor in [0.9, 0.8, 0.1]:
                    rados_obj.rotate_logs(active_mgr)
                    assert mon_config_method.set_config(
                        section="mgr",
