        ...
        try:
            ...
            test_map = [
                {
                    ...
                    the cluster, but its crush entry would be retained with a status change to 'destroyed'. This \
                    OSD's entry will be removed from `ceph node ls` output. No 'CEPHADM_STRAY_DAEMON' warning \
                    should be generated on the cluster.",
                },
                {
                    ...
                    "zap": False,
                    "expectation": "As the device is not being zapped, the concerned OSD will be removed from \
                    the cluster, but its crush entry would be retained with a status change to 'destroyed'. \
                    This OSD's entry will be removed from `ceph node ls` output. No 'CEPHADM_STRAY_DAEMON' \
                ...
                },
                {
                    ...
                    removed from the cluster, but its crush entry would be retained with a status change to \
                    'destroyed' and device will be available for OSD re-deployment, hence the OSD will be added \
                    back and won't remain in destroyed state. This OSD's entry will NOT be removed from \
                    `ceph node ls` output. No 'CEPHADM_STRAY_DAEMON' warning should be generated on the cluster.",
                },
                {
                    ...
                    "expectation": "As the device is being zapped and replaced, the concerned OSD will be \
                    removed from the cluster, but its crush entry would be retained with a status change to \
                    'destroyed' and device will be available for OSD re-deployment, however, as the service \
                    is unmanaged, the OSD would remain in 'destroyed' state. This OSD's entry will be removed \
                    from `ceph node ls` output. No 'CEPHADM_STRAY_DAEMON' warning should be generated on the cluster.",
                },
            ]
            ...
            for test in test_map:
                ...
                if test["zap"] and not test["unmanaged"]:
                    ...
                    assert osd_id in destroyed_osds, (
                        "OSD.%s is not in destroyed state" % osd_id
                    )
            ...
            for key, val in osd_devices.items():
                for dev in val:
                    utils.zap_device(
                        ceph_cluster=ceph_cluster, host=key, device_path=dev
                    )
