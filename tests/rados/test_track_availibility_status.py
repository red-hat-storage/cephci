...
def run(ceph_cluster, **kw) -> int:
    ...
    try:
        ...
        if "case11" in config.get("case_to_run"):
            log.info(
                "=== Case 11: Verification of track availability tests using create_data_pool ==="
            )
            osd_object_map = {}
            assert create_data_pool(mon_obj, rados_object, replicated_config, pool_name)
            method_should_succeed(
            ...
            log.info(
                "=== Case 11: Verification of track availability tests using create_data_pool completed ==="
            )
        if "case12" in config.get("case_to_run"):
            log.info(
                "===Case 12: Verification of track availability tests by rebooting the osd node  ===="
            )
            assert create_data_pool(mon_obj, rados_object, replicated_config, pool_name)
            method_should_succeed(
                wait_for_clean_pg_sets, rados_object, test_pool=pool_name, timeout=1800
            ...
            log.info("Case 12: Rebooting the primary osd node")
            ...
            log.info(
                "Case 12: PG's are active + clean post reboot of host %s",
                osd_host.hostname,
            )

            if not test_all_values(client, pool_name):
                msg_error = (
                    f"Case 12: The track availability tests are failed on {pool_name} after performing the scrub "
                    f"operation"
                )
                log.error(msg_error)
                return 1
            ...
            log.info(
                "===Case 12: Verification of track availability tests by rebooting the osd node completed  ===="
            )
