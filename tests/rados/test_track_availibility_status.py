...
def run(ceph_cluster, **kw) -> int:
    ...
    try:
        ...
        current_replicate_size = int(output["size"])
        ...
        method_should_succeed(
            wait_for_clean_pg_sets, rados_object, test_pool=pool_name, timeout=900
        )
        ...
        if not result_all_tests:
            ...
            log.error(msg_error)
            return 1
        cmd_revert__replica_size = (
            f"ceph osd pool set {pool_name} size {current_replicate_size} "
        )
        rados_object.run_ceph_command(cmd_revert__replica_size)
        method_should_succeed(
            wait_for_clean_pg_sets, rados_object, test_pool=pool_name, timeout=900
        )
        ...
        if availability_msg is not out_put:
            msg_error = (
...
def get_pool_availability_status(client):
    ...
    if type(exact_data) is not tuple:
...
def test_score_value(client, pool_name):
    ...
    total_down_time = time_to_seconds(pool_data[1])
    no_of_failures = int(pool_data[2])
    expected_score = int(pool_data[5])
