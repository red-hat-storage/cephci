    ...
    try:
        ...
        if "case3" in config.get("case_to_run"):
            ...
            if not recovering_found:
                log.error(
                    "Case3:The cluster is not in recovery state. Not executing the further tests"
                )
                return 1
            method_should_succeed(
                wait_for_clean_pg_sets, rados_object, test_pool=pool_name, timeout=1800
        ...
        if "case6" in config.get("case_to_run"):
            ...
            if inactive_count > 5:
                log.error(
                    " Case 6:Observed multiple PGs in inactive state during PG scale up. Fail"
                )
                return 1
            ...
            if inactive_count > 5:
                log.error(
                    "Case 6: Observed multiple PGs in inactive state during PG scale down. Fail"
                )
                return 1
...
def test_score_value(client, pool_name):
    ...
    if MTBF == 0:
        ...
        ratio = MTBF / (MTBF + MTTR)
        actual_score = round(ratio, 3)
    msg_output = f"The actual score output is {actual_score}"
    log.info(msg_output)
    if not math.isclose(actual_score, expected_score, rel_tol=1e-3, abs_tol=1e-3):
        msg_err = (
