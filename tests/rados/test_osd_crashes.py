    ...
    if config.get("verify_allocator_corruption"):
        ...
        try:
            if _config.get("issue_reproduction"):
                ...
                # )
                script_loc = (
                    "https://raw.githubusercontent.com/harshkumarRH/cephci/"
                    "2335317/utility/generate_frag_objs.py"
                )
                ...
                for size in obj_sizes:
                    ...
                    log.info("Output of ceph-bluestore-tool blues-log-dump: \n")
                    log.info(out)

                    for line in out.splitlines():
                        if "op_file_update  file(ino 27" in line:
                            assert (
                                f"{1000 * index} extents" in line
                ...
                for line in out.splitlines():
                    if "op_file_update  file(ino 27" in line:
                        assert (
                            f"{1000 * (index+1)} extents" in line
                        ), "expected extents not found"
                        ino_lines.append(line)
