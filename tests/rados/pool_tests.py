            ...
            for i in pool_configs:
                ...
                if float(rhbuild.split("-")[0]) >= 7.1:
                    ...
                    if not POOL_APP_NOT_ENABLED:
                        log.error(
                            "Warnings not generated on the cluster for app not enabled."
                        )
                        raise Exception(
                ...
                if not POOL_TOO_FEW_PGS:
                    log.error("Warnings not generated on the cluster for too few PGs.")
                ...
                if not POOL_TOO_MANY_PGS:
                    log.error("Warnings not generated on the cluster for too many PGs.")
                ...
                if not POOL_PG_NUM_NOT_POWER_OF_TWO:
                    log.error(
                        "Warnings not generated on the cluster for non power of 2 no of PGs"
                    )
                    raise Exception("warning not generated by cluster")
                ...
                if MANY_OBJECTS_PER_PG:
                    log.error(
                        "Warnings not removed on the cluster for too many objects per pg, post PG autoscaler ON"
                    )
                    raise Exception("warning generated by cluster")
                log.info(
    ...
    if config.get("Verify_degraded_pool_min_size_behaviour"):
        ...
        for entry in pool_target_configs.values():
            ...
            try:
                ...
                log.info(
                    f"At size = min_size, writes would possible as well as reads."
                ...
                if pool_obj.do_rados_put(client=client_node, pool=pool_name, nobj=10):
                    log.error(
                ...
                if not pool_obj.do_rados_put(
                    client=client_node, pool=pool_name, nobj=5
                ):
                    log.error(
                ...
                if pool_obj.do_rados_put(client=client_node, pool=pool_name, nobj=5):
                    log.error(
