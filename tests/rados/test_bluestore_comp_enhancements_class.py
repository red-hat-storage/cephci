...
class BluestoreDataCompression:
    ...
    def write_io_and_fetch_log_lines(
        ...
        cmd = (
            f"curl -L https://raw.githubusercontent.com/s-vipin/test/refs/heads/main/convert.py -o ~/convert.py; "
    ...
    def set_unset_osd_config_and_restart(self, osd_id, param, value, set=True):
        if set:
    ...
    def create_pool_wrapper(self, pool_name: str):
        ...
        and disables PG autoscaling. It then waits for the pool's pg_num to be
        confirmed as 1, retrying the configuration if necessary. This is useful
        for test scenarios that require a pool with a single PG for predictable
        data placement and OSD selection.

        The method will:
        1. Create the pool with pg_num=1, disable_pg_autoscale=True, and app_name="rbd"
        2. Continuously verify and set pg_num and pgp_num to 1 until confirmed
        3. Wait up to 600 seconds for the pool configuration to stabilize
    ...
    def partial_overwrite(self, **kwargs):
        ...
        compression_mode = kwargs.get("compression_mode", "force")
        pool_name = kwargs["pool_name"]
        alloc_hint = kwargs.get(
            "alloc_hint",
            BLUESTORE_ALLOC_HINTS.SEQUENTIAL_READ & BLUESTORE_ALLOC_HINTS.APPEND_ONLY,
        )
        ...
        info_msg = (
            ...
            f"\n[overwrite] object size -> {write_size} write offset -> {write_size//2}\n{separator}"
        )
        log.info(info_msg)
        ...
        self.set_unset_osd_config_and_restart(
            primary_osd_id, "bluestore_compression_min_blob_size_hdd", 65536, set=True
        )
        log.info(
            "Set config osd.%s bluestore_compression_min_blob_size_hdd=%s"
            % (primary_osd_id, 65536)
        )
        ...
        self.set_unset_osd_config_and_restart(
            primary_osd_id, "bluestore_compression_min_blob_size_hdd", 65536, set=False
        )
        log.info(
            "Removed config osd.%s bluestore_compression_min_blob_size_hdd=%s"
            % (primary_osd_id, 65536)
        )
        ...
        log.info(
            "Removed config osd.%s bluestore_recompression_min_gain=%s"
            % (primary_osd_id, recompression_min_gain)
        )
    ...
    def inbetween_overwrite(self, **kwargs):
        """
        Perform a partial-overwrite recompression validation.
        ...
        2. Sets the per-OSD `bluestore_recompression_min_gain`.
        3. Creates an RBD image and writes initial IO to it.
        4. Performs a inbetween overwrite
        5. Fetches OSD logs and verifies recompression behavior by scanning for
           `Estimator::` and `recompress:` log patterns and evaluating them
           against `recompression_min_gain`.
        ...
        Raises:
            Exception: On failures such as pool creation, enabling compression,
                       image creation, IO failures, or recompression validation failures.
        """
        log.info(self.inbetween_overwrite.__doc__)
        compression_algorithm = kwargs.get("compression_algorithm", "snappy")
        compression_mode = kwargs.get("compression_mode", "force")
        pool_name = kwargs["pool_name"]
        recompression_min_gain = kwargs.get("recompression_min_gain", 1.2)
        ...
        info_msg = (
            ...
            f"\nrbd image name -> {rbd_img}"
            f"\n[write] object size -> {write_size} write offset -> {0}"
            f"\n[overwrite] object size -> {write_size//2} write offset -> {write_size//4}\n{separator}"
        )
        log.info(info_msg)
        ...
        log.info(
            "\n %s \n [STARTING] Step4 : Perform overwrite operation of size %s at offset %s \n %s"
            % (separator, write_size // 2, write_size // 4, separator)
        )

        # offset => 500000/4 -> 125000
        # size => 500000/2 -> 250000
        # [0......................................................500,000]
        #            [125000.....................375000]
        overwrite_logs = self.write_io_and_fetch_log_lines(
            write_utility_type=IOTools.FIO,
            primary_osd_id=primary_osd_id,
            pool_name=pool_name,
            object_size=write_size // 2,  # 500000/2 -> 250000
            blob_size=write_size // 2,
            rbd_img=rbd_img,
            patterns=["bluecompr", "_do_put_new_blobs", "Estimator", "recompress:"],
            write_offset=write_size // 4,  # 500000/4 -> 125000
        )

        log.info(
            "\n [COMPLETED] Step4 : Perform overwrite operation of size %s at offset %s"
            % (write_size // 2, write_size // 4)
        )
        log.info(
            "\n %s \n [STARTING] Step5 : Validating recompression \n %s"
            % (separator, separator)
    ...
    def min_alloc_size_test(self, **kwargs):
        ...
        for offset in kwargs.get("min_alloc_size_variations"):  # default -> [-10, 10]
            ...
            log.info("Removed config bluestore_min_alloc_size for osd.%s" % primary_osd_id)
