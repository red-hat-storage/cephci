...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        assert mon_obj.verify_set_config(
        ...
        for _ in range(10):
            ...
            rados_obj.client.exec_command(sudo=True, cmd=cmd, timeout=600)
            time.sleep(10)
        ...
        out, _ = cephadm.shell([f"ceph tell osd.{primary_osd} compact"])
        log.info(out)
        ...
        actual_db_max_cmd = f"echo {out}" + " | awk '/DB.*[0-9]/ {print $4}' | tail -1"

        real_db = rados_obj.client.exec_command(cmd=real_db_cmd)
        actual_db = rados_obj.client.exec_command(cmd=actual_db_cmd)
        real_db_max = rados_obj.client.exec_command(cmd=real_db_max_cmd)
        actual_db_max = rados_obj.client.exec_command(cmd=actual_db_max_cmd)

        assert real_db == actual_db, "Actual DB != Real DB"
        assert real_db_max == actual_db_max, "Actual maximus DB != Real maximus DB"
