...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        if enable_rgw_thrashing:
            log.info("Setting up RGW S3 client for thrashing...")
            rgw_config = setup_rgw_s3_client(client_node, rados_obj)
            if not rgw_config:
        ...
        if config.get("cleanup_pools", True) and created_pools:
            ...
            with cf.ThreadPoolExecutor(max_workers=3) as cleanup_executor:
                ...
                rgw_cleanup = (
                    cleanup_executor.submit(cleanup_rgw_s3, client_node, rgw_config)
                    if rgw_config
                    else None
                )
...
def setup_rgw_s3_client(
    client_node, rados_obj: RadosOrchestrator
    ...
    Returns:
        Dict with access_key, secret_key, endpoints, buckets, uid or None if setup fails
    """
    ...
    try:
        ...
        def create_new_user(user_id: str) -> dict:
            ...
            cmd = (
                f"radosgw-admin user create --uid={user_id} "
                f"--display-name='OSD Thrash Test User' --email={user_id}@test.local"
            )
            out, _ = client_node.exec_command(cmd=cmd, sudo=True)
            return json_module.loads(out)

        try:
            out, _ = client_node.exec_command(cmd="radosgw-admin user list", sudo=True)
            existing_users = json_module.loads(out) if out.strip() else []
            ...
            for existing_uid in existing_users:
                if existing_uid.startswith("thrash-user-"):
                    out, _ = client_node.exec_command(
                        cmd=f"radosgw-admin user info --uid={existing_uid}", sudo=True
                    )
                    user_info = json_module.loads(out)
...
"""
        ...
        if bucket_name not in out or versioned_bucket not in out:
            log.warning("Bucket creation may have failed, continuing anyway")

        log.info(
            f"RGW S3 client setup complete: {len(rgw_endpoints)} endpoints, "
            f"buckets: {bucket_name} (normal), {versioned_bucket} (versioned)"
        )
        return {
            "access_key": access_key,
            "secret_key": secret_key,
...
def cleanup_rgw_s3(client_node, rgw_config: Dict[str, Any]) -> None:
    """
