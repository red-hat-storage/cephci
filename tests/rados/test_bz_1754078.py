        ...
        with parallel() as p:
            for ceph in ceph_nodes:
                if ceph.role == role:
                    ...
                    out, err = ceph.exec_command(cmd='sudo ceph osd ls-tree {host}'.format(host=ceph.hostname))
                    osd_id_list_on_node = out.read().decode().split()
                    log.info("osds on node {}".format(ceph.hostname))
                    log.info(osd_id_list_on_node)
                    osd_mem_target = check_osd_memory_target_of_node(ceph, osd_id_list_on_node)
                    log.info("Node {a} osd_memory_target in bytes: {b}".format(a=ceph.hostname, b=osd_mem_target))
                    p.spawn(check_osd_daemon_memory_usage, ceph, osd_id_list_on_node, osd_mem_target)
                    time.sleep(1)
...
def check_osd_daemon_memory_usage(osd_node, osd_id_list_on_node, osd_mem_target):
    ...
    """
    log.info("checking memory usage of osds for 180 seconds on {}".format(osd_node.hostname))
    timeout = 180
    while timeout:
        for osd_id in range(len(osd_id_list_on_node)):
            cmd = 'ps aux | grep "/usr/bin/ceph-osd -f --cluster ceph --id {} "'.format(osd_id_list_on_node[osd_id])
            ...
            log.info("osd {a} current memory usage:{b}".format(a=osd_id_list_on_node[osd_id], b=current_mem_usage))
            if current_mem_usage > osd_mem_target:
                raise Exception("osd {} daemon not under memory target,exiting!".format(osd_id_list_on_node[osd_id]))
            log.info("osd memory usage under limit for osd {}".format(osd_id_list_on_node[osd_id]))
        timeout = timeout - 1
...
def check_osd_memory_target_of_node(osd_node, osd_id_list_on_node):
    ...
    """
    cmd = "sudo ceph daemon osd.{id} config show -f=json-pretty".format(id=osd_id_list_on_node[0])
