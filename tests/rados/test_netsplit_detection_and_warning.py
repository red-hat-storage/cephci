...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        if (
            stretch_enabled_checks(rados_obj)
            and regular_cluster_with_mon_location is False
        ):
            STRETCH_MODE = True
        ...
        if REGULAR_CLUSTER:
            ...
            stretch_site = [list() * 2]
            stretch_site[0] = [node[i].hostname for i in range(len(node) // 2)]
            stretch_site[1] = [
                node[i].hostname for i in range(len(node) // 2, len(node))
            ]
        ...
        if ("scenario1" in scenarios_to_run) and (
            STRETCH_MODE
        ...
        if ("scenario2" in scenarios_to_run) and (
            STRETCH_MODE or REGULAR_CLUSTER_WITH_MON_LOCATION or NAZ_STRETCH_CLUSTER
        ):
            info_msg = (
                f"\n{separator}"
        ...
        if ("scenario3" in scenarios_to_run) and (
            STRETCH_MODE
            or REGULAR_CLUSTER_WITH_MON_LOCATION
            or NAZ_STRETCH_CLUSTER
            or REGULAR_CLUSTER
            ...
            info_msg = (
                f"\n{separator}"
                f"\nScenario 3 -> [{logging_code}] Netsplit between mon1 of DC2 and mon2 of DC1. "
        ...
        if "scenario4" in scenarios_to_run and STRETCH_MODE:
            info_msg = (
                f"\n{separator}"
        ...
        if "scenario5" in scenarios_to_run and (
            REGULAR_CLUSTER_WITH_MON_LOCATION or NAZ_STRETCH_CLUSTER or REGULAR_CLUSTER
        ):
            info_msg = (
                f"\n{separator}"
        ...
        if "scenario6" in scenarios_to_run and NAZ_STRETCH_CLUSTER:
            info_msg = (
                f"\n{separator}"
