...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        log.info(f"The {pool_name} pool is created")
        rados_object.bench_write(pool_name=pool_name, max_objs=5000)
        log.info(f"The data is pushed into the {pool_name} pool")
        pg_id_list = rados_object.get_pgid(pool_name=pool_name)
        log.info(f"The pg list of the pool {pool_name} is-- {pg_id_list}")
        ...
        # Check for log
        exec_end_time = datetime.datetime.now() + datetime.timedelta(seconds=20)
        ...
        exec_end_time = datetime.datetime.now() + datetime.timedelta(seconds=90)
        while exec_end_time > datetime.datetime.now():
            end_time, _ = installer.exec_command(
                cmd="sudo date '+%Y-%m-%dT%H:%M:%S.%3N+0000'"
            )
            ...
            for pg_id in pg_id_list:
                ...
                if verify_scrub_log(
                    ...
                    log.info(
                        f"The scrub started on the {pg_id} after unsetting no_deepscrub flag"
                    )
...
def verify_scrub_log(rados_object, init_time, end_time, pgid, log_lines):
    ...
    """
    log.info(f"The log lies to check - {log_lines}")
    acting_set = rados_object.get_pg_acting_set(pg_num=pgid)
    log.info(f"The acting osd set of the pg {pgid} is {acting_set}")
        ...
        for log_line in log_lines:
            for osd_line in osd_lines:
                if re.search(log_line, osd_line):
                    log.info(f"The search string is -{log_line}")
                    log.info(
                        f" Found the log lines on OSD : {osd_id} and log line is {osd_line}"
                    )
                    # The flag is included to check the log lines in all OSDs
                    scrub_flag = True
        if scrub_flag:
            return True
        log.debug(f"The {osd_id} osd logs are - {osd_logs}")
