...
def connect_vcenter(gateway, config, nguid_list):
    # Connect to vCenter
    vcenter_hostname = config.get("vcenter_host")
    vcenter_username = config.get("vcenter_user")
    vcenter_password = config.get("root_password")
    si = connect_to_vcenter(vcenter_hostname, vcenter_username, vcenter_password)
    content = si.RetrieveContent()
    output = "VMWare vCenter Connection Sucess"
    LOG.info(output)
    ...
    Example:
          - test:
              ...
              config:
                gw_node: node5
                vmware_clients:
          	  - vcenter_host: cephqe-vcenter2.lab.eng.blr.redhat.com
            	    ip: 10.70.44.249
            	    root_password: Redhat231#
                    sub_nqn: nqn.2016-06.io.spdk:cnode1
                    sub_port: 4420
              desc: test to configure VMware esx server and validate connection
              destroy-cluster: false
              module: test_ceph_nvmeof_vmware_vcenter_clients.py
              name: Configure VMware vcenter clients
              polarion-id:
    """
    ...
    try:
        ...
        if config.get("vmware_clients"):
            for vmware_client in config["vmware_clients"]:
                ...
                connect_vcenter(gw_node, vmware_client, nguid_list)
