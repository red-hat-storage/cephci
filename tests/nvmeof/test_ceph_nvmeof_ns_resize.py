...
def verfiy_auto_resize(ns_data, nvmegwcli, autoresize_flag):
    for image in ns_data.keys():
        img_args = {"subsystem": f"{ns_data[image]['ns_subsystem_nqn']}"}
        ...
        if ns:
            ...
            if str(autoresize_flag) == str(resize_flag):
                LOG.info(
                    f"{ns.get('rbd_image_name')} disable_auto_resize is set to {resize_flag} as expected"
                )
            else:
                raise Exception(
                    f"{ns.get('rbd_image_name')} disable_auto_resize is set \
                    to {resize_flag} but expected is {autoresize_flag}"
                )
...
def resize_rbd_image(ns_data, rbd_obj, rbd_pool, size_increase, flag="increase"):
    for image in ns_data.keys():
        ...
        LOG.info(f"Resize the RBD image - {ns_data[image]['rbd_image_name']}")
        ...
        LOG.info(
            f"Verify RBD image size is increased for RBD image - {ns_data[image]['rbd_image_name']}"
        )
...
def verfiy_namespace_size(ns_data, nvmegwcli, size_to_verify):
    for image in ns_data.keys():
        ...
        if ns:
            ...
            if size_to_verify == f"{image_size}G":
                LOG.info(
                    f"{ns.get('rbd_image_name')} size is equal to expected size as {size_to_verify}"
                )
            else:
                raise Exception(
                    f"{ns.get('rbd_image_name')} size is not equal to expected size \
                    as {size_to_verify} after image resize"
                )
...
def execute_io(config, ha, ceph_cluster, ns_data):
    ...
    for io_client in initiators:
        ...
        for path in paths:
            client.node.exec_command(cmd=f"blkdiscard {path}", sudo=True)
...
def check_io_percent(ns_data, ha, io_size):
    for image in ns_data.keys():
        ...
        if int(size_to_check) == int(out["used_size"]):
            LOG.info(f"In Image {image} used_size is {io_size}G")
        else:
            raise Exception(
                f"In Image {image} used_size is {out['used_size']}, \
                actual provisioned_size is {out['provisioned_size']}"
            )
...
def test_ceph_83627178(ceph_cluster, config):
    ...
    for image in ns_data.keys():
        ns_auto_resize_args = {
            "subsystem": f"{ns_data[image]['ns_subsystem_nqn']}",
            "nsid": ns_data[image]["nsid"],
        }
