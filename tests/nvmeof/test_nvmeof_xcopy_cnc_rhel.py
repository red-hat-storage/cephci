...
Config-driven operations:
  ...
  - spec_cnc_params_exercise (orch-spec rate/chunk/parallel;
    verifies conf applied + rate-limiter throughput band)

Suite ``ana_affinity`` (same | different) selects src/dst namespaces by ANA /
load-balancing group. Pair with ``lb_group`` on bdevs. Ops emit ``CNC_RESULT``
log lines for same-ANA vs different-ANA comparison.
"""

from ceph.ceph import Ceph
from tests.nvmeof.workflows.cnc import OPERATIONS, enable_cnc_logging
...
def run(ceph_cluster: Ceph, **kwargs) -> int:
    ...
    try:
        ...
        initiator = clients[0]
        # Always enable nvmf_cnc logging before the op so xcopy shows in podman logs
        enable_cnc_logging(nvme_service.gateways, config=config)
        ...
        OPERATIONS[operation](initiator, nvme_service.gateways, config, **op_kwargs)
        return 0
