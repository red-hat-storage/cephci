...
from tests.nvmeof.workflows.initiator import validate_io
...
def scale_down(nvme_service, orch, gateway_nodes_to_be_scaleddown):
    ...
    to_be_scaledown_gws, operational_gws = nvme_service.catogorize(
    ...
    old_namespaces = nvme_service.fetch_namespaces(gateway, ana_ids)
    ...
    # nvme_service.config["gw_nodes"] = gwnodes_to_be_deployed
    nvme_service.gw_nodes = get_nodes_by_ids(
        nvme_service.ceph_cluster, gwnodes_to_be_deployed
    )
    # for i, _ in enumerate(nvme_service.config.get("subsystems", [])):
    #     nvme_service.config["subsystems"][i]["listeners"] = gwnodes_to_be_deployed
    nvme_service.deploy()
    ...
    for gateway in to_be_scaledown_gws:
        ...
        if nvme_service.ceph_cluster.rhcs_version == "8":
            ...
            for w in WaitUntil():
                ...
                if nvme_service.check_gateway_availability(
                    gateway.ana_group_id, orch, state="DELETING"
                ):
                    LOG.info(f"[ {gateway} ] NVMeofGW service is UNAVAILABLE.")
                    active = nvme_service.get_optimized_state(
                        orch, gateway.ana_group_id
                    )
...
def validate_scaleup(nvme_service, orch, scaleup_nodes, namespaces):
    ...
    for gateway in new_gws:
        ...
        for w in WaitUntil(timeout=60):
            ...
            if nvme_service.check_gateway_availability(gateway.ana_group_id, orch):
                LOG.info(f"[ {gateway} ] NVMeofGW service is AVAILABLE.")
                state = nvme_service.get_optimized_state(orch, gateway.ana_group_id)
        ...
        if nvme_service.ceph_cluster.rhcs_version == "8.1":
            time.sleep(60)
            validate_ns_balance = validate_auto_loadbalance(
                orch, nvme_service.pool, nvme_service.group
            )
            LOG.info(f"Validated namespaces in each GW:{validate_ns_balance}")
    ...
    for scaleup_node in scaleup_nodes:
        for gw_node in gw_nodes:
            if gw_node.node.id == scaleup_node:
                gw = nvme_service.check_gateway(gw_node.node.id)
                scaleup_gw = nvme_service.check_gateway(scaleup_node)
