...
class NamespaceMasking:
    ...
    def __init__(
        ...
        # Track executor and io_tasks state
        self.executor = None
        self.io_tasks = []
    ...
    def execute_io_ns_masking(self, init_nodes, images, negative=False):
        ...
        for node in init_nodes:
            ...
            initiator = [client for client in self.clients if client.node.id == node][0]
            self.io_tasks.append(self.executor.submit(initiator.start_fio, "25%"))
            time.sleep(20)

            self.validate_io(images_node, negative=negative)
        return self.executor, self.io_tasks
