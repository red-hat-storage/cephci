...
def validate_qos(client, device, **kw):
    ...
    try:
            ...
            for line in lines:
                ...
                if found_header:
                    ...
                    if len(parts) >= 6 and parts[0] == device:
                        mb_read = float(parts[2]) * 100  # MB_read/s
                        mb_write = float(parts[8]) * 100  # MB_wrtn/s
                        mb_write_iops = float(parts[7]) * 100  # MB_w/s
                        mb_read_iops = float(parts[1]) * 100  # MB_rs
        ...
        if "rw-megabytes-per-second" in kw:
            ...
            total_bw = [
                r + w for r, w in zip(bandwidth["mb_read/s"], bandwidth["mb_write/s"])
            ]
            if all(rw < max_rw_mb for rw in total_bw):
                print(
                    f"QoS validated for {device}: Read+Write values {total_bw} "
                    f"are below {kw['rw-megabytes-per-second']} MB/s."
                )
            else:
                print(
                    f"QoS validation failed for {device}: Read+Write values {total_bw} "
                    f"exceed {kw['rw-megabytes-per-second']} MB/s at least once."
                )
