...
def install_prereq(
    ...
    if firewall:
        # Enable Firewall
        if not enable_firewall(ceph):
            raise FirewallConfigError("Failed to enable Firewall")
        log.info("Firewall enable successfully")

        # Check for Firewall
        if not is_firewall_enabled(ceph):
            raise FirewallConfigError("Firewall not enabled")
        log.info("Firewall is enabled")
