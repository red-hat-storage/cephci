            ...
            for encryption_type in kw.get("config", {}).get("encryption_type", {}):
                for image_name, image_conf in multi_image_config.items():
                    try:
                        ...
                        passphrase = (
                            f"{encryption_type}_passphrase_"
                            + random_string(len=3)
                            + ".bin"
                        )
                        ...
                        log.info(
                            "md5sum checksum is same on both clusters after migration"
                        )
                        ...
                        if source_spec_path:
                            log.info(
                                "Cleaning up source spec path: " + source_spec_path
                            )
...
def run(**kw):
    ...
    try:
        ...
        if rbd_obj:
            ...
            if ret_val == 0:
                log.info(
                    "Testing RBD image migration with external ceph native format with encyption Passed"
                )
