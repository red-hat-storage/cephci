...
def migration_with_raw_data_format(rbd_obj, client, **kw):
    ...
    for pool_type in rbd_obj.get("pool_types"):
        ...
        for pool, pool_config in multi_pool_config.items():
            ...
            if image_size == "0G":
                log.error(f"Image size Verification failed for {image}")
                return 1
...
def migrate_check_consistency(rbd, pool, image, **kw):
    ...
    data_integrity_spec = {
        ...
        "second": {
            "image_spec": f"{kw[pool]['target_pool']}/{kw[pool]['target_image']}",
            "rbd": rbd,
            "client": kw["client"],
            "file_path": f"/tmp/{random_string(len=3)}",
        },
    ...
    if rc:
        log.error(
            f"Data consistency check failed for {kw[pool]['target_pool']}/{kw[pool]['target_image']}"
        )
        return 1
    else:
...
def run_IO(rbd, pool, image, **kw):
    ...
    if err:
        log.error(f"Map, mount and run IOs failed for {pool}/{image}")
        return 1
    else:
        log.info(f"Map, mount and IOs successful for {pool}/{image}")

    out, err = rbd.map(**{"image-or-snap-spec": f"{pool}/{image}"})
    if err:
        log.error(f"Failed to map the source image {pool}/{image}")
        return 1
    else:
        log.info(f"Successfully mapped the source image {pool}/{image}")
    ...
    try:
        ...
        if rbd_obj:
            ...
            if migration_with_raw_data_format(rbd_obj, client, **kw):
                return 1
            log.info("Test live migration with raw data format is successful")

    except Exception as e:
        log.error(f"Test live migration with raw data format failed: {str(e)}")
        return 1
