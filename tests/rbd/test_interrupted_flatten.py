...
def interrupted_flatten_test(rbd, client, pool_type, **kw):
    ...
    try:
        ...
        for i in range(iterations):
            ...
            # Run flatten in background, sleep, then kill it.
            cmd = f"nohup rbd flatten {clone_spec} > /dev/null 2>&1 & pid=$!; sleep {sleep_time}; kill -9 $pid || true"
            ...
            if "parent:" not in out:
                log.warning(
                    f"Flatten finished too quickly (sleep_time={sleep_time}s). "
                    "Could not verify interruption. Consider increasing image size or adjusting sleep."
                )
            else:
                log.info(
