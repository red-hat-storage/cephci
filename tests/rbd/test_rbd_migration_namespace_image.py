"""Module to verify successful live migration of RBD namespace level images from
one ceph cluster to another ceph cluster with native data format.

Test case covered -
CEPH-83597865 - Live migration of images with namespace
...
5. Create a namespace for the pool on the source cluster and then Create one RBD image inside that namespace
...
15. Repeat the above test for EC pool with namespace
"""
...
def test_rbd_live_migration_namespace_image(rbd_obj, c1_client, c2_client, **kw):
    """
    Test to perform live migration of namespace level images with native data format.
    Args:
        rbd_obj: rbd object
        ...
        for pool, pool_config in multi_pool_config.items():
            ...
            for image_name, image_conf in multi_image_config.items():
                ...
                try:
                    ...
                    if err:
                        raise Exception(
                            "Failed to create image: " + image_name + ", err: " + err
                        )
                    log.info(
                    ...
                    md5_sum_before_migration = get_md5sum_rbd_image(
                        image_spec=f"{pool}/{namespace_name}/{image_name}",
                        rbd=rbd,
                        client=c1_client,
                    ...
                    target_namespace = "target_namespace" + random_string(len=5)
                    rc = create_namespace_and_verify(
                        **{
                    ...
                    if rc != 0:
                        raise Exception("Error creating namespace in target pool")
                    ...
                    rbd.migration.prepare_import(
                        ...
                        dest_spec=f"{target_pool}/{target_namespace}/{target_image}",
                        cluster_name=c2,
                    )
                    ...
                    if verify_migration_state(
                        action="prepare",
                        image_spec=f"{target_pool}/{target_namespace}/{target_image}",
                        cluster_name=c2,
                        client=c1_client,
                        **kw,
                    ):
...
def run(**kw):
    """
    Test to execute Live migration on namepsace level rbd images with native data format
    ...
    """
    log.info("Executing CEPH-83597865 - Live migration of images with namespace")
    ...
    try:
        ...
        if configure_common_client_node(
            ...
            raise Exception("Common client node configuration failed")

        if rbd_obj:
            log.info("Executing test on Replicated and EC pool")

            ret_val = test_rbd_live_migration_namespace_image(
                rbd_obj=rbd_obj,
            ...
            if ret_val == 0:
                log.info(
                    "Testing RBD namespace image migration with external ceph native format Passed"
                )

    except Exception as e:
        log.error(
            "RBD namespace level image migration with external ceph native format failed with the error "
            + str(e)
        )
        ret_val = 1
