"""Module to verify successful  live migration of images with external qcow data source as
RAW data format from one ceph cluster to another ceph cluster with an NBD source

Test case covered -
CEPH-CEPH-83598692: Live migration of sparse import with an NBD source of qcow image as raw data format
...
7. Create a QCOW2 formatted data for the RBD mapped disk externally (use qemu-img)
8. Map the RBD image from Cluster1. Start the NBD server using the  qcow image
9. Create an NBD source spec file for the qcow image
10. Execute prepare migration with import-only option with --source-spec-path
...
def test_external_nbd_migration(rbd_obj, c1_client, c2_client, **kw):
    ...
    source of QCOW data format from external ceph cluster
    Args:
        rbd_obj: rbd object
        ...
        for pool, pool_config in multi_pool_config.items():
            ...
            for image_name, image_conf in multi_image_config.items():
                try:
                    ...
                    out, err = rbd.map(
                        **{
                            "image-or-snap-spec": f"{pool}/{image_name}",
                            "device-type": "nbd",
                        }
                    )
                    device = out.strip()

                    qcow_file_path = "/tmp/qcowimage_" + random_string(len=3) + ".qcow2"
                    out, err = c1_client.exec_command(
                        sudo=True,
                        cmd=f"qemu-img convert -f raw -O qcow2  {device} {qcow_file_path}",
                    )
                    if err:
                        raise Exception("Failed to Create a QCOW2 formatted data")
                        return 1
                    else:
                        log.info("Successfully created a QCOW2 formatted data")
                    # Note: The image itself is QCOW, but qemu-nbd -f qcow2 presents it as a raw image
                    out, err = c1_client.exec_command(
                        sudo=True,
                        cmd=(
                            f"qemu-nbd -f qcow2 --read-only --shared 10 --persistent --fork "
                            f"--socket {unix_socket_path} {qcow_file_path}"
                        ),
                    )
                    if err:
                        raise Exception(
                            "Failed to export the qcow image as NBD server over unix socket"
                        )
                        return 1
                    else:
                        log.info(
                            "Successfully exported the qcow image as NBD server over unix socket"
                        )
                    ...
                    # Execute prepare migration for external cluster
                    ...
                    if md5_sum_before_migration != md5_sum_after_migration:
                        raise Exception(
                            "Data on the migrated qcow image is inconsistent"
                        )
                    else:
                        log.info(
                            "Data on the migrated qcow image is consistent with the source"
                        )
...
def run(**kw):
    ...
    """
    log.info(
        "Executing CEPH-83598692: Live migration of sparse import "
        "with an NBD source of qcow data format"
    )
    ...
    try:
        ...
        log.error(
            f"RBD image migration with NBD source as QCOW format failed with the error {str(e)}"
        )
        ret_val = 1
