"""Module to verify successful Live migration of images with snapshots and clones from
one ceph cluster to another ceph cluster with native data format.

Test case covered -
CEPH-83597864 - Live migration of images with snapshots and clones
...
8. Protect the snapshot and clone the snapshot into a new image then create snap for the cloned image
9. Create a cloned source spec file for migration as below
[root@ceph-rbd1-sangadi-bz-1w14jg-node2 ~]#  jq . /tmp/clone_spec
[root@ceph-rbd1-sangadi-bz-1w14jg-node2 ~]# jq . /tmp/clone_spec
{
  "cluster_name": "c1",
  "type": "native",
...
10. Execute prepare migration with import-only option with --source-spec-path for RBD image migration
...
def test_migration_clones(rbd_obj, c1_client, c2_client, **kw):
    """
    Live migration of images with snapshots and clones
    Args:
        rbd_obj: rbd object
            ...
            for image_name, image_conf in multi_image_config.items():
                ...
                try:
                    ...
                    if "failed to Protect snapshot" in out + err:
                        log.error(f"Snapshot Protection failed for {pool}/{image_name}")
                        return 1
                    else:
                        log.info(
                            f"Successfully Protected snapshot {snap_name} for {pool}/{image_name}"
                        )
                    ...
                    if err:
                        log.error(
                            f"Clone creation failed for {pool}/{image_name}@{snap_name} with error {err}"
                        )
                        return 1
                    else:
...
def run(**kw):
    """
    Test to execute Live migration of images with snapshots and clones
    Args:
        kw: Key/value pairs of configuration information to be used in the test
            Example::
            config:
    ...
    """
    log.info(
        "Executing CEPH-83597864 - Live migration of images with snapshots and clones"
    )
    ...
    try:
        ...
        if rbd_obj:
            ...
            if ret_val == 0:
                log.info(
                    "Testing Live migration of images with snapshots and clones Passed"
                )

    except Exception as e:
        log.error(
            f"RBD image live migration of images with snapshots and clones failed with the error {str(e)}"
        )
        ret_val = 1
