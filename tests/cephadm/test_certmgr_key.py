...
def run(ceph_cluster, **kw):
    ...
    if action == "set":
        ...
        # get the node where the key needs to be set
    ...
    if action == "get":
        ...
        # get the scope of the certificate
        _scope = config.get("scope")
        # get the service for which cert needs to be retrieved
        _service = config.get("service")
    ...
    if action == "remove":
        ...
        # get the scope of the certificate
        _scope = config.get("scope")
        # get the service for which key needs to be retrieved
        _service = config.get("service")
        ...
        out = CephAdm(installer).ceph.orch.certmgr.rm_key(
            key_name=_key_name, hostname=hostname, service_value=_service
        )
