...
def run(**kwargs):
    ...
    with Manager() as manager:
        ...
        log.info(f"Parallel test cases: {parallel_tcs}")
        test_rc = 0
...
def execute(test, args, results, parallel_tcs):
    ...
    tc = copy.deepcopy(args.get("tc"))
    tc["polarion-id-link"] = (
        f"https://polarion.engineering.redhat.com/polarion/#/project/CEPH/workitem?id="
        f"{test.get('polarion-id')}"
    )
    ...
    log.info(f"Log File location for test {test_name}: {log_url}")
