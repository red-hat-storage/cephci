...
class REST(object):
    ...
    def __init__(self, **kwargs):
        ...
        base_url = "https://{0}:{1}".format(self._ip, self._port)
    ...
    def __send_request(self, **kwargs):
        ...
        retry_count = 1
        log.info(
            "REST call Details %s: %s, headers=%s, data=%s"
            % (req_type.upper(), main_uri, headers, data)
        )
        while retry_count <= max_retries:
            ...
            if response.status_code == requests.codes.UNAUTHORIZED:
                log.debug(
                    "UNAUTHORIZED ERROR(%s:%s). Please check given credentials: %s"
                    % (response.status_code, response.text, auth)
                )
                raise HTTPError(
                    "(%s) Credential used are %s" % (response.status_code, auth)
                )
            ...
            if response.status_code == requests.codes.BAD_REQUEST:
                ...
                log.debug(
                    "BAD REQUEST(%s): api = %s" % (response.status_code, api_used)
                )
                raise HTTPError(
                    "(%s) Check the api content: %s" % (response.status_code, api_used)
                )

            if response.status_code == requests.codes.UNSUPPORTED_MEDIA_TYPE:
                message = (
                    "%s: Check if the request has been serialized before sending"
            ...
            if retry_count == max_retries:
                ...
                raise CommandExecutionError(
                    ...
                    "Response:[%s:%s] "
                    % (max_retries, req_type, response.status_code, response.text)
                )
