...
def apply_v2_only_mon_addrs(shell_fn: Callable) -> List[str]:
    """
    ...
    Returns:
        List of failure messages; empty when all mons were updated successfully.
    """
    failures: List[str] = []
    ...
    try:
        ...
        msg = f"Failed to parse mon dump JSON: {exc}"
        LOG.error(msg)
        return [msg]

    mons = dump.get("mons") or []
    if not mons:
        msg = "No mons in mon dump; cannot apply v2-only set-addrs"
        LOG.error(msg)
        return [msg]
    ...
    for mon in mons:
        ...
        if not parsed:
            msg = f"Could not resolve name/IP for mon entry: {mon}"
            LOG.error(msg)
            failures.append(msg)
            continue
        name, ip = parsed
        ...
        try:
            ...
            msg = f"mon set-addrs {name} failed: {exc}"
            LOG.error(msg)
            failures.append(msg)

    if failures:
        LOG.error("v2-only mon set-addrs had %s failure(s)", len(failures))
        return failures
