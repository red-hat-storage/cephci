...
def osd_scenario1(osd, devices_dict, dmcrypt=False):
    ...
    vgname = vgcreate(
        osd, osd.LvmConfig.vg_name % "1", devices_dict.get("devices")
    )  # all /dev/vd{b,c,d,e}
    data_lv1 = lvcreate(
        osd,
    ...
    scenario = (
        "{{'data':'{datalv1}','data_vg':'{vg_name}'}},"
        "{{'data':'{datalv2}','data_vg':'{vg_name}',"
        "'db':'{dblv1}','db_vg':'{vg_name}'}},"
        "{{'data':'{datalv3}','data_vg':'{vg_name}',"
        "'wal':'{wallv1}','wal_vg':'{vg_name}'}},"
        "{{'data':'{datalv4}','data_vg':'{vg_name}',"
        "'db':'{dblv2}','db_vg':'{vg_name}','wal':'{wallv2}',"
        "'wal_vg':'{vg_name}'}}".format(
            vg_name=vgname,
            datalv1=data_lv1,
            datalv2=data_lv2,
            dblv1=db_lv1,
...
def osd_scenario1_dmcrypt(osd, devices_dict):
    ...
    return {
        "scenario": generated_sce_dict.get("scenario"),
...
def osd_scenario2(osd, devices_dict, dmcrypt=False):
    ...
    scenario = (
        "{{'data':'{vdb1}','db':'{vdb2}','wal':'{vdb3}'}},"
        "{{'data':'{vdc}','db':'{vdb4}','wal':'{vdb5}'}},"
        "{{'data':'{vdd}'}}".format(
...
def osd_scenario3(osd, devices_dict, dmcrypt=False):
    ...
    pvcreate(osd, devices_dict.get("devices"))
    devs = "{a} {b}".format(
        a=devices_dict.get("device0"), b=devices_dict.get("device2")
    )  # vdb vdd
    ...
    scenario = (
        "{{'data':'{vdb}','db':'{dblv1}','db_vg':'{vgname}',"
        "'wal':'{wallv1}','wal_vg':'{vgname}'}},"
        "{{'data':'{datalv1}','data_vg':'{vgname}','db':'{vdd2}',"
        "'wal':'{vdd3}'}},{{'data':'{vdd1}'}}".format(
