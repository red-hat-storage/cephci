...
def setup_gklm_prereq(ceph_cluster, cloud_type, ibm_iam_token_filename):
    """setup GKLM authentication certs on the rgw nodes, redeploy rgw to mounted gklm certs path and set ceph configs for sse-kms-kmip"""
    ...
    gklm_auth_key_path = "/usr/local/gklm/rgwselfsigned.cert"
    for rgw_node_obj in rgw_nodes:
        rgw_node = rgw_node_obj.node
        log.info(f"installing kafka on node {rgw_node.ip_address}")
        rgw_node.exec_command(sudo=True, cmd="mkdir /usr/local/gklm")
        if cloud_type == "openstack":
            rgw_node.exec_command(
                sudo=True,
                cmd=f"curl -o {gklm_auth_cert_path} http://magna002.ceph.redhat.com/cephci-jenkins/gklm/rgwselfsigned.cert",
            )
            rgw_node.exec_command(
                sudo=True,
                cmd=f"curl -o {gklm_auth_key_path} http://magna002.ceph.redhat.com/cephci-jenkins/gklm/rgwselfsigned.key",
            )
        elif cloud_type == "ibmc":
            ibmc_secrets_base_url = "https://b4fc7484-5e05-41a3-a3bd-bcb72f6fc466.us-south.secrets-manager.appdomain.cloud/api/v2/secret_groups/cephci-rgw-configs/secret_types/arbitrary/secretsmanager.appdomain.cloud/api/v2/secret_groups/cephci-rgw-configs/secret_types/arbitrary/secrets"
            iam_token = "<IBM_CLOUD_IAM_TOKEN>"
            with open(ibm_iam_token_filename, "r") as file:
                iam_token = file.read()
            rgw_node.exec_command(
                sudo=True,
                cmd=f'curl -X GET --location --header "Authorization: Bearer {iam_token}" --header "Accept: application/json" "{ibmc_secrets_base_url}/rgw-self-signed-cert" -o {gklm_auth_cert_path}',
            )
            rgw_node.exec_command(
                sudo=True,
                cmd=f'curl -X GET --location --header "Authorization: Bearer {iam_token}" --header "Accept: application/json" "{ibmc_secrets_base_url}/rgw-self-signed-cert" -o {gklm_auth_key_path}',
            )
    ...
    rgw_node.exec_command(
        sudo=True,
        cmd="ceph config set rgw_crypt_kmip_client_cert /usr/local/gklm/rgwselfsigned.cert",
    )
    rgw_node.exec_command(
        sudo=True,
