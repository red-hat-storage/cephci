"""
Usage:
  xunit2gsheet.py <gsheet_id> <xml_file_path> <ceph_version>
  xunit2gsheet.py (-h | --help)

Options:
  -h --help          Show this help message and exit
  <gsheet_id>        Google Sheets ID to update (found in the sheet URL)
  <xml_file_path>    Path to xUnit XML test results file
  <ceph_version>     Ceph version to associate with these test results

Requirements:
  - A service account JSON file named 'service_account.json' must be
  present in the working directory with Google Sheets API access permissions.
  - Python packages: gspread, pandas, xml.etree.ElementTree, google-auth

Examples:
  # Update sheet ABC123 with test results from results.xml for Ceph 1.2.3
  googlespread.py ABC123 results.xml 1.2.3
...
def auto_adjust_gsheet_column_width(worksheet, df):
    ...
    for i, col_name in enumerate(df.columns, 1):
        try:
            ...
            print(f"Could not auto-resize column {i} ({col_name}): {str(e)}")
    # Execute all requests in one batch if there are any
    if requests:
...
def update_gsheet(
    ...
    try:
        ...
        print(f"Error merging summary data: {e}, using new data only.")
        df_summary_final = df_new_summary
    ws_summary.clear()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(GOOGLE_CREDENTIALS_PATH):
        ...
        print(
            "Please download your service account JSON key "
            "and update the GOOGLE_CREDENTIALS_PATH variable."
        )
        sys.exit(1)

    if len(sys.argv) != 4:
        print(
            "Usage: python googlespread.py "
            "<gsheet_id> <xml_file_path> <ceph_version>"
        )
        print("Example: python googlespread.py ABC123 xunit_results.xml 1.2.3")
        sys.exit(1)
    gsheet_id = sys.argv[1]
    xml_file_path = sys.argv[2]
    ceph_version_arg = sys.argv[3]
    print(
        f"Processing {xml_file_path} for version"
        "{ceph_version_arg} into GSheet ID {gsheet_id}"
    )
