...
REMOTE_CONTAINER_LIMITS_SPEC = "/tmp/odf_container_limits.yaml"
...
def apply_container_resource_limits(shell_fn: Callable, installer_node) -> None:
    ...
    Raises ``RuntimeError`` when orch apply fails (avoids silent no-op).
    May still fail on small VMs due to resource constraints.
    """
...
"""
    ...
    remote = remote_node.remote_file(
        sudo=True, file_name=REMOTE_CONTAINER_LIMITS_SPEC, file_mode="w"
    )
    remote.write(spec)
    remote.flush()
    out = _shell(
        ...
        "apply",
        "-i",
        REMOTE_CONTAINER_LIMITS_SPEC,
        check_status=False,
    )
    if not (out or "").strip() or "error" in (out or "").lower():
        raise RuntimeError(f"orch apply container limits failed: {out!r}")
...
def apply_msgr2_only(shell_fn: Callable) -> None:
    ...
    failures = apply_v2_only_mon_addrs(shell_fn)
    if failures:
        raise RuntimeError("v2-only mon set-addrs failed: " + "; ".join(failures))
    _shell(
        shell_fn,
        "ceph",
        "config",
