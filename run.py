...
def create_nodes(
    ...
    for cluster in conf.get("globals"):
        ...
        for node in ceph_vmnodes.values():
            ...
            if cloud_type == "openstack":
                ...
                private_ip = node.ip_address
                ceph_nodename = node.hostname
                # Auth: 1) private_key_path -> connect as onecloud-user (bootstrap), setup cephuser, switch
                # Hardened images only allow publickey; onecloud-user has our key from cloud-init.
                # 2) root+passwd (userData) for non-hardened images
                if onecloud_cfg.get("private_key_path"):
                    ssh_username = "onecloud-user"
                    root_username = "onecloud-user"
                    root_password = ""
                    look_for_key = True
            ...
            if node.role == "win-iscsi-clients":
                ...
                root_pwd = (
                    ""
                    if (ssh_username == "onecloud-user" and look_for_key)
    ...
    if cloud_type == "onecloud":
        # Always wait for cloud-init when userData is injected (both key and root+passwd paths)
        wait_sec = 75
        log.info("OneCloud: sleeping %ds for cloud-init", wait_sec)
        time.sleep(wait_sec)
    else:
